echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/postgres_17/run.sh' >> /etc/sudoers
zfs create tank/fire/postgres_17
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/postgres_17/vmlinux.bin

<<'EOF' cat > /mnt/fire/postgres_17/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/postgres_17/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/postgres_17/vsock /mnt/fire/postgres_17/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/postgres_17/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/postgres_17/run.sh

mkdir /mnt/fire/postgres_17/mnt

<<'EODOCKER' cat > /mnt/fire/postgres_17/DOCKERFILE
...
apt-get update
apt-get -y install postgresql-17 postgresql-17-postgis-3 postgresql-17-pgrouting postgresql-plpython3-17
<<'EOF' cat > /etc/postgresql/17/main/pg_ident.conf
# MAPNAME     SYSTEM-USERNAME   PG-USERNAME
map           postgres          postgres
map           root              postgres
EOF
<<'EOF' cat > /etc/postgresql/17/main/pg_hba.conf
#TYPE   DATABASE    USER        CIDR-ADDRESS          METHOD
local   all         postgres                          ident map=map
EOF
<<'EOF' cat > /etc/postgresql/17/main/postgresql.conf
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_postgres_17 - < /mnt/fire/postgres_17/DOCKERFILE
#docker run --rm -ti dummy_postgres_17
# 850 = du 754M + ~28M ext4 overhead + ~68M free
dd if=/dev/zero bs=1M count=850 > /mnt/fire/postgres_17/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/postgres_17/rootfs.ext4
mount -o loop /mnt/fire/postgres_17/rootfs.ext4 /mnt/fire/postgres_17/mnt
docker run --rm -ti -v /mnt/fire/postgres_17/mnt:/my-rootfs dummy_postgres_17
...
cat <<"EOF" > /mnt/fire/postgres_17/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/postgres_17/mnt/fiddle.sh

#mount -o loop /mnt/fire/postgres_17/rootfs.ext4 /mnt/fire/postgres_17/mnt
<<'EOC' cat > /mnt/fire/postgres_17/fiddle.c
...
cp /mnt/fire/postgres_17/fiddle.c /mnt/fire/postgres_17/mnt/fiddle.c
docker run --rm -v /mnt/fire/postgres_17/mnt:/my-rootfs dummy_postgres_17 -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends gcc libc6-dev libpq-dev && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/postgres_17/mnt /usr/bin/ldd /fiddle

cp /mnt/fire/postgres_17/vsock /mnt/fire/postgres_17/mnt/vsock
chmod 755 /mnt/fire/postgres_17/mnt/vsock

umount /mnt/fire/postgres_17/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/postgres_17/rootfs.ext4 2>/dev/null | awk '
...
zfs set recordsize=16K tank/fire/postgres_17

cd /mnt/fire/postgres_17
rm -f mem vmstate v.sock* /tmp/fc-snap-postgres_17.sock /tmp/fc-snap-postgres_17.log
firecracker-1.13 --api-sock /tmp/fc-snap-postgres_17.sock --config-file config.json > /tmp/fc-snap-postgres_17.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-postgres_17.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-postgres_17.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-postgres_17.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/postgres_17@base 2>/dev/null || true
zfs snapshot tank/fire/postgres_17@base

out=$(echo '["select 1"]' | /mnt/fire/postgres_17/run.sh) || true
printf '%s\n' "$out"
