echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/oracle_23c/run.sh' >> /etc/sudoers
zfs create tank/fire/oracle_23c
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/oracle_23c/vmlinux.bin

# no-kvmapf: a task asleep on an async page fault at snapshot time never wakes after restore
<<'EOF' cat > /mnt/fire/oracle_23c/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/oracle_23c/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/oracle_23c/vsock /mnt/fire/oracle_23c/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/oracle_23c/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/oracle_23c/run.sh

mkdir /mnt/fire/oracle_23c/mnt

cat > /mnt/fire/oracle_23c/DOCKERFILE <<"EOF"
FROM oraclelinux:8
RUN dnf -y install systemd cronie tmux util-linux perl-interpreter hostname
...
# the free RPM's unversioned name has served two builds: the digest pins this one. preinstall
# comes by URL, as ol8_developer is not defined here, and first: %pre needs the user it creates.
RUN curl -fLo /tmp/preinstall.rpm https://yum.oracle.com/repo/OracleLinux/OL8/developer/x86_64/getPackage/oracle-database-preinstall-23c-1.0-0.5.el8.x86_64.rpm \
  && curl -fLo /tmp/oracle-free.rpm https://download.oracle.com/otn-pub/otn_software/db-free/oracle-database-free-23c-1.0-1.el8.x86_64.rpm \
  && printf '%s  /tmp/preinstall.rpm\n%s  /tmp/oracle-free.rpm\n' \
       2c88437a537e28527a068ef540d0c4ec30ff9b5890d57d9493a3d1bd0b533e60 \
       1319bcd7cb706cb727501cbd98abf3f3980a4fdabeb613a1abffc756925c7374 | sha256sum -c - \
  && dnf -y install /tmp/preinstall.rpm \
  && ORACLE_DOCKER_INSTALL=true dnf -y localinstall /tmp/oracle-free.rpm \
  && rm /tmp/preinstall.rpm /tmp/oracle-free.rpm
RUN echo /opt/oracle/product/23c/dbhomeFree/lib > /etc/ld.so.conf.d/oracle.conf && ldconfig
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_oracle_23c - < /mnt/fire/oracle_23c/DOCKERFILE
dd if=/dev/zero bs=1M count=18432 > /mnt/fire/oracle_23c/rootfs.ext4
mkfs.ext4 /mnt/fire/oracle_23c/rootfs.ext4
mount -o loop /mnt/fire/oracle_23c/rootfs.ext4 /mnt/fire/oracle_23c/mnt
# --hostname is baked into listener.ora and tnsnames.ora: it must match the guest's /etc/hosts
docker run --rm -ti --hostname fiddle --shm-size 2g -v /mnt/fire/oracle_23c/mnt:/my-rootfs dummy_oracle_23c
(echo wXYyXLlDVZB0ss2F; echo wXYyXLlDVZB0ss2F) | /etc/init.d/oracle-free-23c configure
export ORACLE_HOME=/opt/oracle/product/23c/dbhomeFree ORACLE_SID=FREE
$ORACLE_HOME/bin/sqlplus sys/wXYyXLlDVZB0ss2F@//localhost:1521/free as sysdba <<"SQL"
alter pluggable database freepdb1 save state;
exit
SQL
$ORACLE_HOME/bin/sqlplus sys/wXYyXLlDVZB0ss2F@//localhost:1521/freepdb1 as sysdba <<"SQL"
...
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/oracle_23c/mnt/etc/hosts
echo fiddle > /mnt/fire/oracle_23c/mnt/etc/hostname
rm -f /mnt/fire/oracle_23c/mnt/etc/systemd/system/multi-user.target.wants/oracle-free-23c.service

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/oracle_23c/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/oracle_23c/mnt/fiddle.sh

<<'EOP' cat > /mnt/fire/oracle_23c/mnt/probe.c
...
EOP
<<'EOC' cat > /mnt/fire/oracle_23c/mnt/fiddle.c
...
static int connect_db(void){
  ...
  return OCILogon2(env, err, &svc,
                   ...
                   (const OraText *)"localhost/freepdb1", 18,
...
EOC
OH=/opt/oracle/product/23c/dbhomeFree
chroot /mnt/fire/oracle_23c/mnt /usr/bin/gcc -O2 -o /probe /probe.c -I$OH/rdbms/public -L$OH/lib -lclntsh
CSID=$(chroot /mnt/fire/oracle_23c/mnt /usr/bin/env ORACLE_HOME=$OH /probe)
chroot /mnt/fire/oracle_23c/mnt /usr/bin/gcc -O2 -Wall -DAL32UTF8_ID=$CSID -o /fiddle /fiddle.c \
  -I$OH/rdbms/public -L$OH/lib -lclntsh
rm -f /mnt/fire/oracle_23c/mnt/fiddle.c /mnt/fire/oracle_23c/mnt/probe.c /mnt/fire/oracle_23c/mnt/probe
chroot /mnt/fire/oracle_23c/mnt /usr/bin/ldd /fiddle

install -m 755 /mnt/fire/oracle_23c/vsock /mnt/fire/oracle_23c/mnt/vsock

umount /mnt/fire/oracle_23c/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
free_mb=$(dumpe2fs -h /mnt/fire/oracle_23c/rootfs.ext4 2>/dev/null \
...
zfs set recordsize=16K tank/fire/oracle_23c

( cd /mnt/fire/oracle_23c
  rm -f mem vmstate v.sock* /tmp/fc-snap-oracle_23c.sock /tmp/fc-snap-oracle_23c.log
  firecracker-1.13 --api-sock /tmp/fc-snap-oracle_23c.sock --config-file config.json > /tmp/fc-snap-oracle_23c.log 2>&1 &
  until grep -q FIDDLE-READY /tmp/fc-snap-oracle_23c.log ; do sleep 0.1 ; done
  curl -sf --unix-socket /tmp/fc-snap-oracle_23c.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
  curl -sf --unix-socket /tmp/fc-snap-oracle_23c.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
  kill $! || true )
# a cold snapshot just serves slowly: check the warm-up ran
grep -a 'warm-up:' /tmp/fc-snap-oracle_23c.log

# never -R: on a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/oracle_23c@base 2>/dev/null || true
zfs snapshot tank/fire/oracle_23c@base

cd /
out=$(echo '["select 1 from dual"]' | /mnt/fire/oracle_23c/run.sh) || true
printf '%s\n' "$out"
