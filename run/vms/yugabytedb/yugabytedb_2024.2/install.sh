echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/yugabytedb_2024.2/run.sh' >> /etc/sudoers
zfs create tank/fire/yugabytedb_2024.2
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/yugabytedb_2024.2/vmlinux.bin

<<'EOF' cat > /mnt/fire/yugabytedb_2024.2/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/yugabytedb_2024.2/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/yugabytedb_2024.2/vsock /mnt/fire/yugabytedb_2024.2/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/yugabytedb_2024.2/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/yugabytedb_2024.2/run.sh

mkdir /mnt/fire/yugabytedb_2024.2/mnt

<<'EODOCKER' cat > /mnt/fire/yugabytedb_2024.2/DOCKERFILE
FROM debian:bookworm-slim
RUN apt-get update -y \
  && apt-get install -y --no-install-recommends \
       systemd-sysv udev wget ca-certificates python3 python-is-python3 \
       libpq5 procps netbase locales \
  && sed -i "s/^# *en_US.UTF-8 UTF-8/en_US.UTF-8 UTF-8/" /etc/locale.gen \
  && locale-gen en_US.UTF-8 \
  && rm -rf /var/lib/apt/lists/*

RUN wget -qO - https://downloads.yugabyte.com/releases/2024.2.10.0/yugabyte-2024.2.10.0-b62-linux-x86_64.tar.gz \
      | tar -zx \
  && mv yugabyte-2024.2.10.0 yugabyte
WORKDIR /yugabyte
RUN ./bin/post_install.sh

RUN echo '* - nofile 1048576' > /etc/security/limits.d/99-yugabyte.conf \
  && printf '%s\n' '[Service]' 'ExecStart=/fiddle.sh' '[Install]' 'WantedBy=default.target' \
       > /etc/systemd/system/fiddle.service \
  && systemctl enable fiddle \
  && echo "root:Docker!" | chpasswd

# stop cleanly, or the snapshot freezes the ceremony's boot mid rocksdb/WAL recovery.
# The second start and stop is not redundant: it prunes ~320M the first leaves behind.
RUN set -eux; \
    ./bin/yugabyted start \
      --master_flags="durable_wal_write=false,reject_writes_min_disk_space_mb=20" \
      --tserver_flags="reject_writes_min_disk_space_mb=20" --listen 127.0.0.1; \
    i=0; while [ $i -lt 180 ]; do \
      /yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433 >/dev/null 2>&1 && break; \
      i=$((i+1)); sleep 1; done; \
    /yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433; \
    ./bin/yugabyted stop; \
    ./bin/yugabyted start \
      --master_flags="durable_wal_write=false,reject_writes_min_disk_space_mb=20" \
      --tserver_flags="reject_writes_min_disk_space_mb=20" --listen 127.0.0.1; \
...
docker build --no-cache -t dummy_yugabytedb_2024.2 - < /mnt/fire/yugabytedb_2024.2/DOCKERFILE
#docker run --rm -ti dummy_yugabytedb_2024.2
# 1727 = 1648M populated + 79M free. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=1727 > /mnt/fire/yugabytedb_2024.2/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/yugabytedb_2024.2/rootfs.ext4
mount -o loop /mnt/fire/yugabytedb_2024.2/rootfs.ext4 /mnt/fire/yugabytedb_2024.2/mnt
docker run --rm -ti -v /mnt/fire/yugabytedb_2024.2/mnt:/my-rootfs dummy_yugabytedb_2024.2
# fallocate takes one filename: \; not +, which silently punches nothing
# never an apostrophe in this block: mkdriver.sh wraps it in single quotes, so one would
# end the quote and run the rest on the HOST
find /root/var -type f -size +1M -exec fallocate -d {} \;
tar -cSp --exclude=/my-rootfs --one-file-system / | tar x -C /my-rootfs
chmod 1777 /my-rootfs/tmp
rm -rf /my-rootfs/run /my-rootfs/proc /my-rootfs/sys
...
<<'EOF' cat > /mnt/fire/yugabytedb_2024.2/mnt/fiddle.sh
#!/bin/sh
dd if=/dev/random count=1 bs=1
# reject_writes_min_disk_space_mb: the unset threshold is larger than this rootfs, so
# without it every write is rejected
/yugabyte/bin/yugabyted start --master_flags="durable_wal_write=false,reject_writes_min_disk_space_mb=20" --tserver_flags="reject_writes_min_disk_space_mb=20" --listen 127.0.0.1
until /yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433 >/dev/null 2>&1 ; do sleep 0.1 ; done
...
# not redundant: after the restore's clock jump the tserver is not serving until pg_isready answers
until /yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433 >/dev/null 2>&1 ; do sleep 0.05 ; done
/fiddle
/vsock reply < /data/output.json
reboot -ff
EOF

chmod 700 /mnt/fire/yugabytedb_2024.2/mnt/fiddle.sh

#mount -o loop /mnt/fire/yugabytedb_2024.2/rootfs.ext4 /mnt/fire/yugabytedb_2024.2/mnt
<<'EOC' cat > /mnt/fire/yugabytedb_2024.2/fiddle.c
/* the postgres family's runner, apart from the conninfo and the search_path: carry a
 * change to both families, or parity against postgres compares runners, not engines.
 * A failed connect writes no output.json on purpose: a yugabyte that did not come back
 ...
 * it back to UTF-8.
 */
#define YB_CONNINFO "host=localhost port=5433 user=yugabyte password=yugabyte"
...
int main(int argc, char **argv){
  ...
  PGconn *conn = PQconnectdb(YB_CONNINFO);
...
cp /mnt/fire/yugabytedb_2024.2/fiddle.c /mnt/fire/yugabytedb_2024.2/mnt/fiddle.c
docker run --rm --entrypoint bash -v /mnt/fire/yugabytedb_2024.2/mnt:/my-rootfs dummy_yugabytedb_2024.2 -c 'apt-get update -qq >/dev/null 2>&1 && apt-get install -y -qq gcc libpq-dev >/dev/null 2>&1 && gcc -O2 -Wall -I/usr/include/postgresql -o /my-rootfs/fiddle /my-rootfs/fiddle.c -L/usr/lib/x86_64-linux-gnu -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/yugabytedb_2024.2/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/yugabytedb_2024.2/mnt /usr/bin/ldd /fiddle | grep -q 'not found' \
  && { echo "ABORT: /fiddle has an unresolved shared library"; exit 1; }
chroot /mnt/fire/yugabytedb_2024.2/mnt /usr/bin/ldd /fiddle | grep -q 'libpq\.so\.5 => /' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_yugabytedb_2024.2

cp /mnt/fire/yugabytedb_2024.2/vsock /mnt/fire/yugabytedb_2024.2/mnt/vsock
chmod 755 /mnt/fire/yugabytedb_2024.2/mnt/vsock

# fallocate takes one filename: \; not +, which silently punches nothing
find /mnt/fire/yugabytedb_2024.2/mnt/root/var -type f -size +1M -exec fallocate -d {} \; 2>/dev/null || true

umount /mnt/fire/yugabytedb_2024.2/mnt

dumpe2fs -h /mnt/fire/yugabytedb_2024.2/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/yugabytedb_2024.2

cd /mnt/fire/yugabytedb_2024.2
rm -f mem vmstate v.sock* /tmp/fc-snap-yugabytedb_2024.2.sock /tmp/fc-snap-yugabytedb_2024.2.log
firecracker-1.13 --api-sock /tmp/fc-snap-yugabytedb_2024.2.sock --config-file config.json > /tmp/fc-snap-yugabytedb_2024.2.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-yugabytedb_2024.2.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2024.2.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2024.2.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-yugabytedb_2024.2.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/yugabytedb_2024.2@base 2>/dev/null || true
zfs snapshot tank/fire/yugabytedb_2024.2@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/yugabytedb_2024.2/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/yugabytedb_2024.2/run.sh) || true
printf '%s\n' "$out"
