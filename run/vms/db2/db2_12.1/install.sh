echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/db2_12.1/run.sh' >> /etc/sudoers
zfs create tank/fire/db2_12.1
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/db2_12.1/vmlinux.bin

# no-kvmapf: a task asleep on an async page fault at snapshot time never wakes after restore
<<'EOF' cat > /mnt/fire/db2_12.1/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/db2_12.1/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/db2_12.1/vsock /mnt/fire/db2_12.1/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/db2_12.1/run.sh
#!/bin/bash
...
EOF
chmod 755 /mnt/fire/db2_12.1/run.sh

# never latest: it crosses into the next release. Sort numerically, or 12.1.9 outranks 12.1.10.
TOK=$(curl -s "https://icr.io/oauth/token?service=registry&scope=repository:db2_community/db2:pull" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
TAG=$(curl -s -H "Authorization: Bearer $TOK" "https://icr.io/v2/db2_community/db2/tags/list" | python3 -c '
import sys, json, re
t = [x for x in json.load(sys.stdin)["tags"] if re.match(r"^12\.1\.", x)]
t.sort(key=lambda v: [int(n) for n in v.split(".")])
print(t[-1])')
[ -n "$TAG" ] || { echo "FAIL: could not resolve a 12.1 tag"; exit 1; }
echo "building db2_12.1 from icr.io/db2_community/db2:$TAG"
docker pull icr.io/db2_community/db2:$TAG

cat > /mnt/fire/db2_12.1/DOCKERFILE <<"EOF"
FROM icr.io/db2_community/db2:__TAG__
...
ENTRYPOINT ["bash"]
EOF
sed -i "s|__TAG__|$TAG|" /mnt/fire/db2_12.1/DOCKERFILE
grep -q "^FROM icr.io/db2_community/db2:12.1." /mnt/fire/db2_12.1/DOCKERFILE || { echo "FAIL: tag substitution"; exit 1; }
mkdir -p /mnt/fire/db2_12.1/ctx
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_db2_12.1 -f /mnt/fire/db2_12.1/DOCKERFILE /mnt/fire/db2_12.1/ctx

# 4625M = 4393M tree + ~155M ext4 metadata + ~72M free. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=4625 > /mnt/fire/db2_12.1/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/db2_12.1/rootfs.ext4
mkdir -p /mnt/fire/db2_12.1/mnt
mount -o loop /mnt/fire/db2_12.1/rootfs.ext4 /mnt/fire/db2_12.1/mnt
...
docker run --rm -ti --hostname fiddle --shm-size 2g --cap-add=IPC_OWNER \
  -v /mnt/fire/db2_12.1/mnt:/my-rootfs dummy_db2_12.1
groupadd -g 5000 db2inst && useradd -u 5000 -g 5000 -m -d /home/db2inst db2inst
groupadd -g 5001 db2fsdm && useradd -u 5001 -g 5001 -m -d /home/db2fsdm db2fsdm
# 12.1 requires -sharedgroup or -nosharedgroup; -nosharedgroup is 11.5's behaviour
/opt/ibm/db2/V12.1/instance/db2icrt -u db2fsdm -nosharedgroup db2inst
runuser -u db2inst -- /bin/bash -lc '
set -e
. /home/db2inst/sqllib/db2profile
# records the resolved mod pack in the build log
db2level
...
db2 get db cfg for fiddle | grep -iE "code set|code page|territory"
db2 terminate
db2stop force'
...
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/db2_12.1/mnt/etc/hosts
echo fiddle > /mnt/fire/db2_12.1/mnt/etc/hostname

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
<<'EOF' cat > /mnt/fire/db2_12.1/mnt/fiddle.sh
#!/bin/sh
...
reboot -ff
EOF
chmod 700 /mnt/fire/db2_12.1/mnt/fiddle.sh

<<'EOC' cat > /mnt/fire/db2_12.1/mnt/fiddle.c
...
chroot /mnt/fire/db2_12.1/mnt /usr/bin/gcc -O2 -Wall -o /fiddle /fiddle.c \
  -I/opt/ibm/db2/V12.1/include -L/opt/ibm/db2/V12.1/lib64 -ldb2 \
  -Wl,-rpath,/opt/ibm/db2/V12.1/lib64
chroot /mnt/fire/db2_12.1/mnt /usr/bin/ldd /fiddle
chmod 755 /mnt/fire/db2_12.1/mnt/fiddle

install -m 755 /mnt/fire/db2_12.1/vsock /mnt/fire/db2_12.1/mnt/vsock

umount /mnt/fire/db2_12.1/mnt
docker rmi dummy_db2_12.1

# read the headroom after the umount: before it the superblock still shows the empty image
free_mb=$(dumpe2fs -h /mnt/fire/db2_12.1/rootfs.ext4 2>/dev/null \
...
zfs set recordsize=16K tank/fire/db2_12.1

( cd /mnt/fire/db2_12.1
  rm -f mem vmstate v.sock* /tmp/fc-snap-db2_12.1.sock /tmp/fc-snap-db2_12.1.log
  firecracker-1.13 --api-sock /tmp/fc-snap-db2_12.1.sock --config-file config.json > /tmp/fc-snap-db2_12.1.log 2>&1 &
  until grep -qa FIDDLE-READY /tmp/fc-snap-db2_12.1.log ; do sleep 0.1 ; done
  curl -sf --unix-socket /tmp/fc-snap-db2_12.1.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
  curl -sf --unix-socket /tmp/fc-snap-db2_12.1.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
  kill $! || true )
grep -a 'warm-up:' /tmp/fc-snap-db2_12.1.log
grep -a 'Kernel command line' /tmp/fc-snap-db2_12.1.log | grep -q no-kvmapf || { echo "FAIL: no-kvmapf did not take"; exit 1; }
grep -qa 'Unknown kernel command line parameters' /tmp/fc-snap-db2_12.1.log && { echo "FAIL: kernel rejected a parameter"; exit 1; }

zfs destroy tank/fire/db2_12.1@base 2>/dev/null || true
zfs snapshot tank/fire/db2_12.1@base

cd /
out=$(echo '["select 1 from sysibm.sysdummy1"]' | /mnt/fire/db2_12.1/run.sh) || true
printf '%s\n' "$out"
