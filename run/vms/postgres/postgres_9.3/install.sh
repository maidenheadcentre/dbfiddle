echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/postgres_9.3/run.sh' >> /etc/sudoers
zfs create tank/fire/postgres_9.3
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/postgres_9.3/vmlinux.bin

<<'EOF' cat > /mnt/fire/postgres_9.3/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/postgres_9.3/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/postgres_9.3/vsock /mnt/fire/postgres_9.3/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/postgres_9.3/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/postgres_9.3/run.sh

mkdir /mnt/fire/postgres_9.3/mnt

<<'EODOCKER' cat > /mnt/fire/postgres_9.3/DOCKERFILE
FROM alpine:3.16
...
docker build --no-cache -t dummy_postgres_9.3 - < /mnt/fire/postgres_9.3/DOCKERFILE
#docker run --rm -ti dummy_postgres_9.3
# 238 = 163M occupied by the populated image + ~75M free. Measure the occupancy from the
# superblock after a throwaway populate, never with du in the builder container.
dd if=/dev/zero bs=1M count=238 > /mnt/fire/postgres_9.3/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/postgres_9.3/rootfs.ext4
mount -o loop /mnt/fire/postgres_9.3/rootfs.ext4 /mnt/fire/postgres_9.3/mnt
docker run --rm -ti -v /mnt/fire/postgres_9.3/mnt:/my-rootfs dummy_postgres_9.3
...
cat <<"EOF" > /mnt/fire/postgres_9.3/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/postgres_9.3/mnt/fiddle.sh

#mount -o loop /mnt/fire/postgres_9.3/rootfs.ext4 /mnt/fire/postgres_9.3/mnt
<<'EOC' cat > /mnt/fire/postgres_9.3/fiddle.c
...
cp /mnt/fire/postgres_9.3/fiddle.c /mnt/fire/postgres_9.3/mnt/fiddle.c
docker run --rm -v /mnt/fire/postgres_9.3/mnt:/my-rootfs dummy_postgres_9.3 -c 'apk add --no-cache gcc musl-dev >/dev/null && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -L$(pg_config --libdir) -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/postgres_9.3/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/postgres_9.3/mnt /usr/bin/ldd /fiddle | grep -q 'libpq.so.5 => /usr/local/lib/libpq.so.5' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_postgres_9.3

cp /mnt/fire/postgres_9.3/vsock /mnt/fire/postgres_9.3/mnt/vsock
chmod 755 /mnt/fire/postgres_9.3/mnt/vsock

umount /mnt/fire/postgres_9.3/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/postgres_9.3/rootfs.ext4 2>/dev/null | awk '
...
zfs set recordsize=16K tank/fire/postgres_9.3

cd /mnt/fire/postgres_9.3
rm -f mem vmstate v.sock* /tmp/fc-snap-postgres_9.3.sock /tmp/fc-snap-postgres_9.3.log
firecracker-1.13 --api-sock /tmp/fc-snap-postgres_9.3.sock --config-file config.json > /tmp/fc-snap-postgres_9.3.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-postgres_9.3.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-postgres_9.3.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-postgres_9.3.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/postgres_9.3@base 2>/dev/null || true
zfs snapshot tank/fire/postgres_9.3@base

out=$(echo '["select 1"]' | /mnt/fire/postgres_9.3/run.sh) || true
printf '%s\n' "$out"
