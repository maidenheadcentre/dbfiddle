...
echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/mysql_9.7/run.sh' >> /etc/sudoers
zfs create tank/fire/mysql_9.7
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/mysql_9.7/vmlinux.bin

<<'EOF' cat > /mnt/fire/mysql_9.7/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/mysql_9.7/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/mysql_9.7/vsock /mnt/fire/mysql_9.7/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/mysql_9.7/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/mysql_9.7/run.sh

mkdir /mnt/fire/mysql_9.7/mnt

<<'EOC' cat > /mnt/fire/mysql_9.7/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
/* lang is "" for the engine's own SQL; never NULL */
...
/* never /usr/bin/python3: on OL9 it stays 3.9 however many interpreters are installed */
#define PYTHON "/usr/bin/python3.12"
...
/* the script goes in a file, not down stdin: a traceback then names the batch */
...
int main(int argc, char **argv){
  ...
  /* 15s, inside run.sh's 20s: a hung python batch must not get the guest killed with
     no body to return */
  ...
  for(size_t i = 0; i < nb; i++){
    ...
    } else if(mysql_real_query(conn, batches[i].s, batches[i].len) == 0){
...
cat > /mnt/fire/mysql_9.7/DOCKERFILE <<"EOF"
FROM mysql:9.7-oraclelinux9
RUN microdnf install -y systemd cronie tmux util-linux perl-interpreter mariadb-connector-c
RUN microdnf install -y oracle-epel-release-el9
RUN microdnf install -y haveged
RUN microdnf install -y python3.12 python3.12-pip
RUN /usr/bin/python3.12 -m pip install --no-cache-dir mysql-connector-python
RUN sed -i 's/user=mysql/user=root/g' /etc/my.cnf
RUN /usr/sbin/mysqld --initialize-insecure \
  && /usr/sbin/mysqld --user=root --skip-networking --daemonize &> /dev/null \
  && sleep 10 \
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_mysql_9.7 - < /mnt/fire/mysql_9.7/DOCKERFILE
#docker run --rm -ti dummy_mysql_9.7
# 1552 = ~1478M used + the 50-100M free target
dd if=/dev/zero bs=1M count=1552 > /mnt/fire/mysql_9.7/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/mysql_9.7/rootfs.ext4
mount -o loop /mnt/fire/mysql_9.7/rootfs.ext4 /mnt/fire/mysql_9.7/mnt
cp /mnt/fire/mysql_9.7/fiddle.c /mnt/fire/mysql_9.7/mnt/fiddle.c
# the builder's release must match the guest's, or the runner links against a glibc the guest lacks
docker run --rm -v /mnt/fire/mysql_9.7/mnt:/my-rootfs oraclelinux:9 bash -c 'dnf install -y -q gcc mariadb-connector-c-devel && gcc -O2 -Wall -o /my-rootfs/fiddle /my-rootfs/fiddle.c $(mariadb_config --cflags --libs) && rm /my-rootfs/fiddle.c'
docker run --rm -ti -v /mnt/fire/mysql_9.7/mnt:/my-rootfs dummy_mysql_9.7
...
docker rmi dummy_mysql_9.7 || true

chroot /mnt/fire/mysql_9.7/mnt /usr/bin/python3.12 -c 'import mysql.connector'

cp /mnt/fire/mysql_9.7/vsock /mnt/fire/mysql_9.7/mnt/vsock
chmod 755 /mnt/fire/mysql_9.7/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/mysql_9.7/mnt/fiddle.sh
#!/bin/sh
...
date -u -s @946684800
/usr/sbin/mysqld --user=root --skip-networking --daemonize > /dev/null 2> /dev/null
...
[["import mysql.connector\nc = mysql.connector.connect(unix_socket='/var/run/mysqld/mysqld.sock', user='root', database='fiddle')\ncur = c.cursor()\ncur.execute('select 1')\nprint(cur.fetchone()[0])\nc.close()","python"]]
JSON
/fiddle /tmp/warm.json /tmp/warm.out
echo "warm-up python: $(cat /tmp/warm.out)" > /dev/console
rm -f /tmp/warm.json /tmp/warm.out /tmp/batch*.py
sync
...
chmod 700 /mnt/fire/mysql_9.7/mnt/fiddle.sh

umount /mnt/fire/mysql_9.7/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/mysql_9.7/rootfs.ext4 2>/dev/null | awk '
...
zfs set recordsize=16K tank/fire/mysql_9.7

cd /mnt/fire/mysql_9.7
rm -f mem vmstate v.sock* /tmp/fc-snap-mysql_9.7.sock /tmp/fc-snap-mysql_9.7.log
firecracker-1.13 --api-sock /tmp/fc-snap-mysql_9.7.sock --config-file config.json > /tmp/fc-snap-mysql_9.7.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-mysql_9.7.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-mysql_9.7.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-mysql_9.7.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
...
zfs destroy tank/fire/mysql_9.7@base 2>/dev/null || true
zfs snapshot tank/fire/mysql_9.7@base

# run.sh has no meaningful exit status: assert on the body
out=$(echo '["select version()"]' | /mnt/fire/mysql_9.7/run.sh) || true
printf '%s\n' "$out"
[ -n "$out" ] || { echo "ABORT: verification fiddle returned an empty body"; exit 1; }

cat > /tmp/mysql_9.7-langcheck.json <<'JSON'
["create table t as select 42 x",
 ["import mysql.connector\nc = mysql.connector.connect(unix_socket='/var/run/mysqld/mysqld.sock', user='root', database='fiddle')\ncur = c.cursor()\ncur.execute('select x from t')\nprint('py-sees:' + str(cur.fetchone()[0]))","python"],
 ["console.log(1)","node"],
 ["import sys\nsys.exit(3)","python"]]
JSON
lang=$(/mnt/fire/mysql_9.7/run.sh < /tmp/mysql_9.7-langcheck.json) || true
printf '%s\n' "$lang"
