echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/oracle_26/run.sh' >> /etc/sudoers
zfs create tank/fire/oracle_26
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/oracle_26/vmlinux.bin

# no-kvmapf: a task asleep on an async page fault at snapshot time never wakes after restore
<<'EOF' cat > /mnt/fire/oracle_26/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/oracle_26/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/oracle_26/vsock /mnt/fire/oracle_26/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/oracle_26/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/oracle_26/run.sh

mkdir /mnt/fire/oracle_26/mnt

cat > /mnt/fire/oracle_26/DOCKERFILE <<"EOF"
FROM oraclelinux:9
RUN dnf -y install systemd oracle-ai-database-preinstall-26ai cronie tmux util-linux perl-interpreter hostname
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_oracle_26 - < /mnt/fire/oracle_26/DOCKERFILE
dd if=/dev/zero bs=1M count=18432 > /mnt/fire/oracle_26/rootfs.ext4
mkfs.ext4 /mnt/fire/oracle_26/rootfs.ext4
mount -o loop /mnt/fire/oracle_26/rootfs.ext4 /mnt/fire/oracle_26/mnt
# --hostname is baked into listener.ora and tnsnames.ora: it must match the guest's /etc/hosts
docker run --rm -ti --hostname fiddle --shm-size 2g -v /mnt/fire/oracle_26/mnt:/my-rootfs dummy_oracle_26
(echo wXYyXLlDVZB0ss2F; echo wXYyXLlDVZB0ss2F) | /etc/init.d/oracle-free-26ai configure
export ORACLE_HOME=/opt/oracle/product/26ai/dbhomeFree ORACLE_SID=FREE
...
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/oracle_26/mnt/etc/hosts
echo fiddle > /mnt/fire/oracle_26/mnt/etc/hostname
rm -f /mnt/fire/oracle_26/mnt/etc/systemd/system/multi-user.target.wants/oracle-free-26ai.service

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/oracle_26/mnt/fiddle.sh
#!/bin/sh
dd if=/dev/random count=1 bs=1
# ksipc needs a non-loopback private ip at startup, else ORA-600 [ksipc: no private ips avail
# for use]. The veth pair has no host side, so the guest stays offline; it needs CONFIG_VETH.
ip link add v0 type veth peer name v1
ip addr add 192.168.100.1/24 dev v0
ip link set v0 up
ip link set v1 up
export ORACLE_HOME=/opt/oracle/product/26ai/dbhomeFree ORACLE_SID=FREE
...
chmod 700 /mnt/fire/oracle_26/mnt/fiddle.sh

<<'EOP' cat > /mnt/fire/oracle_26/mnt/probe.c
...
EOC
OH=/opt/oracle/product/26ai/dbhomeFree
chroot /mnt/fire/oracle_26/mnt /usr/bin/gcc -O2 -o /probe /probe.c -I$OH/rdbms/public -L$OH/lib -lclntsh
CSID=$(chroot /mnt/fire/oracle_26/mnt /usr/bin/env ORACLE_HOME=$OH /probe)
chroot /mnt/fire/oracle_26/mnt /usr/bin/gcc -O2 -Wall -DAL32UTF8_ID=$CSID -o /fiddle /fiddle.c \
  -I$OH/rdbms/public -L$OH/lib -lclntsh
rm -f /mnt/fire/oracle_26/mnt/fiddle.c /mnt/fire/oracle_26/mnt/probe.c /mnt/fire/oracle_26/mnt/probe
chroot /mnt/fire/oracle_26/mnt /usr/bin/ldd /fiddle

install -m 755 /mnt/fire/oracle_26/vsock /mnt/fire/oracle_26/mnt/vsock

umount /mnt/fire/oracle_26/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
free_mb=$(dumpe2fs -h /mnt/fire/oracle_26/rootfs.ext4 2>/dev/null \
...
zfs set recordsize=16K tank/fire/oracle_26

( cd /mnt/fire/oracle_26
  rm -f mem vmstate v.sock* /tmp/fc-snap-oracle_26.sock /tmp/fc-snap-oracle_26.log
  firecracker-1.13 --api-sock /tmp/fc-snap-oracle_26.sock --config-file config.json > /tmp/fc-snap-oracle_26.log 2>&1 &
  until grep -q FIDDLE-READY /tmp/fc-snap-oracle_26.log ; do sleep 0.1 ; done
  curl -sf --unix-socket /tmp/fc-snap-oracle_26.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
  curl -sf --unix-socket /tmp/fc-snap-oracle_26.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
  kill $! || true )
...
zfs snapshot tank/fire/oracle_26@base

cd /
out=$(echo '["select 1 from dual"]' | /mnt/fire/oracle_26/run.sh) || true
printf '%s\n' "$out"
