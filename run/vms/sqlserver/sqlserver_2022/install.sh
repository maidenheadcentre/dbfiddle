...
echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/sqlserver_2022/run.sh' >> /etc/sudoers
zfs create tank/fire/sqlserver_2022
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/sqlserver_2022/vmlinux.bin
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/cpu_template.json
{
  "cpuid_modifiers": [
    {
      "leaf": "0x80000007",
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2022/vsock /mnt/fire/sqlserver_2022/vsock.c
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/reseedrng.c
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2022/reseedrng /mnt/fire/sqlserver_2022/reseedrng.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/sqlserver_2022/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/sqlserver_2022/run.sh

mkdir /mnt/fire/sqlserver_2022/mnt
mkdir /mnt/fire/sqlserver_2022/build
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/build/fiddle.csproj
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/build/fiddle.cs
using System;
...
<<'EOF' cat > /mnt/fire/sqlserver_2022/build/Dockerfile
...
docker build -t dummy_sqlserver_2022 /mnt/fire/sqlserver_2022/build
dd if=/dev/zero bs=1M count=6144 > /mnt/fire/sqlserver_2022/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/sqlserver_2022/rootfs.ext4
mount -o loop /mnt/fire/sqlserver_2022/rootfs.ext4 /mnt/fire/sqlserver_2022/mnt
docker run --rm -i -v /mnt/fire/sqlserver_2022/mnt:/my-rootfs dummy_sqlserver_2022 -s <<"SETUP"
...
docker rmi dummy_sqlserver_2022

cp /mnt/fire/sqlserver_2022/vsock /mnt/fire/sqlserver_2022/mnt/vsock
chmod 755 /mnt/fire/sqlserver_2022/mnt/vsock
cp /mnt/fire/sqlserver_2022/reseedrng /mnt/fire/sqlserver_2022/mnt/reseedrng
chmod 755 /mnt/fire/sqlserver_2022/mnt/reseedrng

# replace docker's bind-mounted network identity with the guest's offline one
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/sqlserver_2022/mnt/etc/hosts
echo fiddle > /mnt/fire/sqlserver_2022/mnt/etc/hostname
: > /mnt/fire/sqlserver_2022/mnt/etc/resolv.conf
rm -f /mnt/fire/sqlserver_2022/mnt/etc/systemd/system/multi-user.target.wants/mssql-server.service

# ---- init boot ----
<<'EOF' cat > /mnt/fire/sqlserver_2022/mnt/fiddle.sh
#!/bin/sh
...
reboot -ff
EOF
chmod 700 /mnt/fire/sqlserver_2022/mnt/fiddle.sh
umount /mnt/fire/sqlserver_2022/mnt

cd /mnt/fire/sqlserver_2022
rm -f /tmp/fc-init-sqlserver_2022.log
firecracker-1.13 --no-api --config-file config.json > /tmp/fc-init-sqlserver_2022.log 2>&1
grep FIDDLE-INIT-DONE /tmp/fc-init-sqlserver_2022.log   # must appear; if not, read the log
grep -a 'FIDDLE-DB-RC=0' /tmp/fc-init-sqlserver_2022.log        # the `fiddle` database was created
grep -a 'FIDDLE-DB-STATE-RC=0' /tmp/fc-init-sqlserver_2022.log  # ...with snapshot isolation and a master key
...
dumpe2fs -h /mnt/fire/sqlserver_2022/rootfs.ext4 2>/dev/null | grep -E 'Block count|Free blocks|Block size|Reserved block count'
...
rm -f mem vmstate v.sock* /tmp/fc-snap-sqlserver_2022.sock /tmp/fc-snap-sqlserver_2022.log
firecracker-1.13 --api-sock /tmp/fc-snap-sqlserver_2022.sock --config-file config.json > /tmp/fc-snap-sqlserver_2022.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-sqlserver_2022.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2022.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2022.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
...
grep -a SQLSERVR-WARM /tmp/fc-snap-sqlserver_2022.log       # must appear, BEFORE FIDDLE-READY
grep -a 'Kernel command line' /tmp/fc-snap-sqlserver_2022.log | grep -a no-kvmapf   # must match
grep -a 'Unknown kernel command line parameters' /tmp/fc-snap-sqlserver_2022.log    # must NOT match
grep -a CLOCK-AGED /tmp/fc-snap-sqlserver_2022.log          # randomness lever 1 ran
grep -a 'RESEEDRNG: .* states re-keyed' /tmp/fc-snap-sqlserver_2022.log   # lever 2's layout still matches
grep -a 'RESEEDRNG: .* 0 states re-keyed' /tmp/fc-snap-sqlserver_2022.log && echo 'CEREMONY FAILED: CNG layout no longer matches'

# leaked clones block the destroy, leaving a stale @base the snapshot line then trips on
zfs destroy tank/fire/sqlserver_2022@base 2>/dev/null || true
zfs snapshot tank/fire/sqlserver_2022@base || echo 'CEREMONY FAILED: stale @base still in place (leaked clones?)'

screen_bad=0
for i in $(seq 1 $SCREEN_N); do
  [ -n "$(echo '["select 1"]' | /mnt/fire/sqlserver_2022/run.sh 2>/dev/null)" ] || screen_bad=$((screen_bad + 1))
done
...
out=$(echo '["select @@version"]' | /mnt/fire/sqlserver_2022/run.sh) || true
printf '%s\n' "$out"
