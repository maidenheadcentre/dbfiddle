echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/documentdb_0.114/run.sh' >> /etc/sudoers
zfs create tank/fire/documentdb_0.114
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/documentdb_0.114/vmlinux.bin

<<'EOF' cat > /mnt/fire/documentdb_0.114/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/documentdb_0.114/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/documentdb_0.114/vsock /mnt/fire/documentdb_0.114/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/documentdb_0.114/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/documentdb_0.114/run.sh

mkdir /mnt/fire/documentdb_0.114/mnt

<<'EODOCKER' cat > /mnt/fire/documentdb_0.114/DOCKERFILE
...
echo "deb https://repo.mongodb.org/apt/debian trixie/mongodb-org/8.0 main" > /etc/apt/sources.list.d/mongodb.list
apt-get update
...
V=$(curl -sf https://api.github.com/repos/documentdb/documentdb/releases | jq -r '.[].tag_name' | grep '^v0\.114-' | sort -V | tail -1)
[ -n "$V" ] || { echo "ABORT: no 0.114 release found"; exit 1; }
echo "documentdb release: $V"
mkdir -p /opt/documentdb
echo "$V" > /opt/documentdb/VERSION
curl -sfLO "https://github.com/documentdb/documentdb/releases/download/$V/deb13-postgresql-18-documentdb_${V#v}_amd64.deb"
apt-get -y install ./deb13-postgresql-18-documentdb_${V#v}_amd64.deb
rm -f deb13-postgresql-18-documentdb_${V#v}_amd64.deb
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_documentdb_0.114 - < /mnt/fire/documentdb_0.114/DOCKERFILE
#docker run --rm -ti dummy_documentdb_0.114
# 1270 = 1200 + 70: a complete tree at 1200 leaves ~5M after the ceremony, and free space
# moves 1:1 with the count. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=1270 > /mnt/fire/documentdb_0.114/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/documentdb_0.114/rootfs.ext4
mount -o loop /mnt/fire/documentdb_0.114/rootfs.ext4 /mnt/fire/documentdb_0.114/mnt
docker run --rm -ti -v /mnt/fire/documentdb_0.114/mnt:/my-rootfs dummy_documentdb_0.114
...
cat <<"EOF" > /mnt/fire/documentdb_0.114/mnt/fiddle.sh
#!/bin/bash
...
reboot -ff
EOF
chmod 700 /mnt/fire/documentdb_0.114/mnt/fiddle.sh

#mount -o loop /mnt/fire/documentdb_0.114/rootfs.ext4 /mnt/fire/documentdb_0.114/mnt
<<'EOC' cat > /mnt/fire/documentdb_0.114/fiddle.c
...
cp /mnt/fire/documentdb_0.114/fiddle.c /mnt/fire/documentdb_0.114/mnt/fiddle.c
docker run --rm -v /mnt/fire/documentdb_0.114/mnt:/my-rootfs dummy_documentdb_0.114 -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends gcc libc6-dev libpq-dev && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/documentdb_0.114/mnt /usr/bin/ldd /fiddle
...
docker run --rm -v /mnt/fire/documentdb_0.114/mnt:/my-rootfs dummy_documentdb_0.114 -c 'set -e
  apt-get update -qq
...
chroot /mnt/fire/documentdb_0.114/mnt /usr/bin/ldd /opt/documentdb/documentdb_gateway

chroot /mnt/fire/documentdb_0.114/mnt /usr/bin/mongosh --version \
...
docker rmi dummy_documentdb_0.114


cp /mnt/fire/documentdb_0.114/vsock /mnt/fire/documentdb_0.114/mnt/vsock
chmod 755 /mnt/fire/documentdb_0.114/mnt/vsock

umount /mnt/fire/documentdb_0.114/mnt

dumpe2fs -h /mnt/fire/documentdb_0.114/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/documentdb_0.114

cd /mnt/fire/documentdb_0.114
# per-engine log and socket: a concurrent ceremony on a shared path steals the snapshot
rm -f mem vmstate v.sock* /tmp/fc-snap-documentdb_0.114.sock /tmp/fc-snap-documentdb_0.114.log
firecracker-1.13 --api-sock /tmp/fc-snap-documentdb_0.114.sock --config-file config.json > /tmp/fc-snap-documentdb_0.114.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-documentdb_0.114.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-documentdb_0.114.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-documentdb_0.114.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-documentdb_0.114.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/documentdb_0.114@base 2>/dev/null || true
zfs snapshot tank/fire/documentdb_0.114@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/documentdb_0.114/rootfs.ext4 "$gate"; then
...
["select documentdb_api.insert_one('test','t','{\"a\":1}')",
 ["db.t.find()", "mongosh"]]
VEOF
out=$(printf '%s' "$verify" | /mnt/fire/documentdb_0.114/run.sh) || true
printf '%s\n' "$out"
