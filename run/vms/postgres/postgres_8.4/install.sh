echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/postgres_8.4/run.sh' >> /etc/sudoers
zfs create tank/fire/postgres_8.4
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/postgres_8.4/vmlinux.bin

<<'EOF' cat > /mnt/fire/postgres_8.4/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/postgres_8.4/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/postgres_8.4/vsock /mnt/fire/postgres_8.4/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/postgres_8.4/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/postgres_8.4/run.sh

mkdir /mnt/fire/postgres_8.4/mnt

<<'EODOCKER' cat > /mnt/fire/postgres_8.4/DOCKERFILE
# syntax=docker/dockerfile:1.3-labs
FROM debian/eol:squeeze
ENV DEBIAN_FRONTEND=noninteractive
RUN <<'EORUN'
<<'EOF' cat > /etc/apt/sources.list
deb http://archive.debian.org/debian squeeze main
deb http://archive.debian.org/debian squeeze-lts main
EOF
apt-get update || true
apt-get install -y --force-yes postgresql-8.4 postgresql-contrib-8.4
<<'EOF' cat > /etc/postgresql/8.4/main/pg_ident.conf
# MAPNAME     SYSTEM-USERNAME   PG-USERNAME
map           postgres          postgres
map           root              postgres
EOF
<<'EOF' cat > /etc/postgresql/8.4/main/pg_hba.conf
#TYPE   DATABASE    USER        CIDR-ADDRESS          METHOD
local   all         postgres                          ident map=map
EOF
# no wal_level, max_wal_senders or max_parallel_workers_per_gather: 8.4 refuses to start on
# an unknown parameter
<<'EOF' cat > /etc/postgresql/8.4/main/postgresql.conf
...
EOF
# plpgsql in both: the runner connects to postgres, and a fiddle's create database copies
# template1. The postinst leaves the cluster running: stop it cleanly, or every guest boots
# into WAL recovery.
pg_ctlcluster 8.4 main stop || true
pg_ctlcluster 8.4 main start
su postgres -c 'createlang plpgsql template1'
su postgres -c 'createlang plpgsql postgres'
su postgres -c "psql -tAc \"select count(*) from pg_language where lanname='plpgsql'\" postgres" | grep -qx 1
pg_ctlcluster 8.4 main stop
grep -q 'shut down' /var/lib/postgresql/8.4/main/pg_log/*.log
EORUN
RUN <<'EORUN'
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_postgres_8.4 - < /mnt/fire/postgres_8.4/DOCKERFILE
#docker run --rm -ti dummy_postgres_8.4
# 316 = 241M occupied by the populated image + ~75M free. Measure the occupancy from the
# superblock after a throwaway populate, never with du in the builder container.
dd if=/dev/zero bs=1M count=316 > /mnt/fire/postgres_8.4/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/postgres_8.4/rootfs.ext4
mount -o loop /mnt/fire/postgres_8.4/rootfs.ext4 /mnt/fire/postgres_8.4/mnt
docker run --rm -ti -v /mnt/fire/postgres_8.4/mnt:/my-rootfs dummy_postgres_8.4
for d in bin etc home lib lib64 opt root sbin usr dev var; do tar c "/$d" | tar x -C /my-rootfs; done
rm -f /my-rootfs/dev/console /my-rootfs/dev/ttyS0
mknod -m 600 /my-rootfs/dev/console c 5 1
mknod -m 660 /my-rootfs/dev/ttyS0 c 4 64
mkdir /my-rootfs/tmp
...
cat <<"EOF" > /mnt/fire/postgres_8.4/mnt/fiddle.sh
#!/bin/sh
dd if=/dev/random count=1 bs=1
until [ -S /var/run/postgresql/.s.PGSQL.5432 ] ; do sleep 0.1 ; done
until psql -U postgres -tAc 'select 1' >/dev/null 2>&1 ; do sleep 0.05 ; done
...
chmod 700 /mnt/fire/postgres_8.4/mnt/fiddle.sh

#mount -o loop /mnt/fire/postgres_8.4/rootfs.ext4 /mnt/fire/postgres_8.4/mnt
<<'EOC' cat > /mnt/fire/postgres_8.4/fiddle.c
...
cp /mnt/fire/postgres_8.4/fiddle.c /mnt/fire/postgres_8.4/mnt/fiddle.c
docker run --rm -v /mnt/fire/postgres_8.4/mnt:/my-rootfs dummy_postgres_8.4 -c 'apt-get update -qq || true; apt-get install -y --force-yes -qq gcc libc6-dev libpq-dev && gcc -O2 -Wall -std=gnu99 -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/postgres_8.4/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/postgres_8.4/mnt /usr/bin/ldd /fiddle | grep -q 'libpq.so.5 => /usr/lib/libpq.so.5' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_postgres_8.4

cp /mnt/fire/postgres_8.4/vsock /mnt/fire/postgres_8.4/mnt/vsock
chmod 755 /mnt/fire/postgres_8.4/mnt/vsock

umount /mnt/fire/postgres_8.4/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/postgres_8.4/rootfs.ext4 2>/dev/null | awk '
...
zfs set recordsize=16K tank/fire/postgres_8.4

cd /mnt/fire/postgres_8.4
rm -f mem vmstate v.sock* /tmp/fc-snap-postgres_8.4.sock /tmp/fc-snap-postgres_8.4.log
firecracker-1.13 --api-sock /tmp/fc-snap-postgres_8.4.sock --config-file config.json > /tmp/fc-snap-postgres_8.4.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-postgres_8.4.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-postgres_8.4.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-postgres_8.4.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/postgres_8.4@base 2>/dev/null || true
zfs snapshot tank/fire/postgres_8.4@base

out=$(echo '["select 1"]' | /mnt/fire/postgres_8.4/run.sh) || true
printf '%s\n' "$out"
