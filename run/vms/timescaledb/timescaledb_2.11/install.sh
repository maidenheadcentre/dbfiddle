echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/timescaledb_2.11/run.sh' >> /etc/sudoers
zfs create tank/fire/timescaledb_2.11
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/timescaledb_2.11/vmlinux.bin

<<'EOF' cat > /mnt/fire/timescaledb_2.11/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/timescaledb_2.11/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/timescaledb_2.11/vsock /mnt/fire/timescaledb_2.11/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/timescaledb_2.11/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/timescaledb_2.11/run.sh

mkdir /mnt/fire/timescaledb_2.11/mnt

<<'EODOCKER' cat > /mnt/fire/timescaledb_2.11/DOCKERFILE
...
apt-get update
apt-get -y install postgresql-15 postgresql-plpython3-15 timescaledb-2-postgresql-15='2.11*' timescaledb-2-loader-postgresql-15='2.11*' timescaledb-toolkit-postgresql-15
<<'EOF' cat > /etc/postgresql/15/main/pg_ident.conf
# MAPNAME     SYSTEM-USERNAME   PG-USERNAME
map           postgres          postgres
map           root              postgres
EOF
<<'EOF' cat > /etc/postgresql/15/main/pg_hba.conf
#TYPE   DATABASE    USER        CIDR-ADDRESS          METHOD
local   all         postgres                          ident map=map
EOF
# the postgres family's conf plus the preload line: keep it in step with postgres_15's
<<'EOF' cat > /etc/postgresql/15/main/postgresql.conf
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_timescaledb_2.11 - < /mnt/fire/timescaledb_2.11/DOCKERFILE
#docker run --rm -ti dummy_timescaledb_2.11
# 810 = 682M populated + 30M mke2fs overhead + 23M ceremony boot + ~75M free
dd if=/dev/zero bs=1M count=810 > /mnt/fire/timescaledb_2.11/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/timescaledb_2.11/rootfs.ext4
mount -o loop /mnt/fire/timescaledb_2.11/rootfs.ext4 /mnt/fire/timescaledb_2.11/mnt
docker run --rm -ti -v /mnt/fire/timescaledb_2.11/mnt:/my-rootfs dummy_timescaledb_2.11
...
cat <<"EOF" > /mnt/fire/timescaledb_2.11/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/timescaledb_2.11/mnt/fiddle.sh

#mount -o loop /mnt/fire/timescaledb_2.11/rootfs.ext4 /mnt/fire/timescaledb_2.11/mnt
<<'EOC' cat > /mnt/fire/timescaledb_2.11/fiddle.c
...
cp /mnt/fire/timescaledb_2.11/fiddle.c /mnt/fire/timescaledb_2.11/mnt/fiddle.c
docker run --rm -v /mnt/fire/timescaledb_2.11/mnt:/my-rootfs dummy_timescaledb_2.11 -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends gcc libc6-dev libpq-dev && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/timescaledb_2.11/mnt /usr/bin/ldd /fiddle
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_timescaledb_2.11

cp /mnt/fire/timescaledb_2.11/vsock /mnt/fire/timescaledb_2.11/mnt/vsock
chmod 755 /mnt/fire/timescaledb_2.11/mnt/vsock

umount /mnt/fire/timescaledb_2.11/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/timescaledb_2.11/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/timescaledb_2.11

cd /mnt/fire/timescaledb_2.11
rm -f mem vmstate v.sock* /tmp/fc-snap-timescaledb_2.11.sock /tmp/fc-snap-timescaledb_2.11.log
firecracker-1.13 --api-sock /tmp/fc-snap-timescaledb_2.11.sock --config-file config.json > /tmp/fc-snap-timescaledb_2.11.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-timescaledb_2.11.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-timescaledb_2.11.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-timescaledb_2.11.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-timescaledb_2.11.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/timescaledb_2.11@base 2>/dev/null || true
zfs snapshot tank/fire/timescaledb_2.11@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/timescaledb_2.11/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/timescaledb_2.11/run.sh) || true
printf '%s\n' "$out"
