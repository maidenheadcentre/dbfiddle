echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/db2_11.5/run.sh' >> /etc/sudoers
zfs create tank/fire/db2_11.5
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/db2_11.5/vmlinux.bin

# no-kvmapf: a task asleep on an async page fault at snapshot time never wakes after restore
<<'EOF' cat > /mnt/fire/db2_11.5/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/db2_11.5/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/db2_11.5/vsock /mnt/fire/db2_11.5/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/db2_11.5/run.sh
#!/bin/bash
...
EOF
chmod 755 /mnt/fire/db2_11.5/run.sh

# never latest: it crosses into the next release. Sort numerically, or 11.5.9 outranks 11.5.10.
...
import sys, json, re
t = [x for x in json.load(sys.stdin)["tags"] if re.match(r"^11\.5\.", x)]
t.sort(key=lambda v: [int(n) for n in v.split(".")])
print(t[-1])')
[ -n "$TAG" ] || { echo "FAIL: could not resolve an 11.5 tag"; exit 1; }
echo "building db2_11.5 from icr.io/db2_community/db2:$TAG"
docker pull icr.io/db2_community/db2:$TAG

cat > /mnt/fire/db2_11.5/DOCKERFILE <<"EOF"
...
ENTRYPOINT ["bash"]
EOF
sed -i "s|__TAG__|$TAG|" /mnt/fire/db2_11.5/DOCKERFILE
grep -q "^FROM icr.io/db2_community/db2:11.5." /mnt/fire/db2_11.5/DOCKERFILE || { echo "FAIL: tag substitution"; exit 1; }
mkdir -p /mnt/fire/db2_11.5/ctx
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_db2_11.5 -f /mnt/fire/db2_11.5/DOCKERFILE /mnt/fire/db2_11.5/ctx

# 3200M for a ~2.95G tree leaves 50-100M free
dd if=/dev/zero bs=1M count=3200 > /mnt/fire/db2_11.5/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/db2_11.5/rootfs.ext4
mkdir -p /mnt/fire/db2_11.5/mnt
mount -o loop /mnt/fire/db2_11.5/rootfs.ext4 /mnt/fire/db2_11.5/mnt
...
docker run --rm -ti --hostname fiddle --shm-size 2g --cap-add=IPC_OWNER \
  -v /mnt/fire/db2_11.5/mnt:/my-rootfs dummy_db2_11.5
...
/opt/ibm/db2/V11.5/instance/db2icrt -u db2fsdm db2inst
...
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/db2_11.5/mnt/etc/hosts
echo fiddle > /mnt/fire/db2_11.5/mnt/etc/hostname

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
<<'EOF' cat > /mnt/fire/db2_11.5/mnt/fiddle.sh
#!/bin/sh
...
reboot -ff
EOF
chmod 700 /mnt/fire/db2_11.5/mnt/fiddle.sh

<<'EOC' cat > /mnt/fire/db2_11.5/mnt/fiddle.c
...
chroot /mnt/fire/db2_11.5/mnt /usr/bin/gcc -O2 -Wall -o /fiddle /fiddle.c \
  -I/opt/ibm/db2/V11.5/include -L/opt/ibm/db2/V11.5/lib64 -ldb2 \
  -Wl,-rpath,/opt/ibm/db2/V11.5/lib64
chroot /mnt/fire/db2_11.5/mnt /usr/bin/ldd /fiddle
chmod 755 /mnt/fire/db2_11.5/mnt/fiddle

install -m 755 /mnt/fire/db2_11.5/vsock /mnt/fire/db2_11.5/mnt/vsock

umount /mnt/fire/db2_11.5/mnt
docker rmi dummy_db2_11.5

# read the headroom after the umount: before it the superblock still shows the empty image
free_mb=$(dumpe2fs -h /mnt/fire/db2_11.5/rootfs.ext4 2>/dev/null \
...
zfs set recordsize=16K tank/fire/db2_11.5

( cd /mnt/fire/db2_11.5
  rm -f mem vmstate v.sock* /tmp/fc-snap-db2_11.5.sock /tmp/fc-snap-db2_11.5.log
  firecracker-1.13 --api-sock /tmp/fc-snap-db2_11.5.sock --config-file config.json > /tmp/fc-snap-db2_11.5.log 2>&1 &
  until grep -qa FIDDLE-READY /tmp/fc-snap-db2_11.5.log ; do sleep 0.1 ; done
  curl -sf --unix-socket /tmp/fc-snap-db2_11.5.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
  curl -sf --unix-socket /tmp/fc-snap-db2_11.5.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
  kill $! || true )
grep -a 'warm-up:' /tmp/fc-snap-db2_11.5.log
grep -a 'Kernel command line' /tmp/fc-snap-db2_11.5.log | grep -q no-kvmapf || { echo "FAIL: no-kvmapf did not take"; exit 1; }
grep -qa 'Unknown kernel command line parameters' /tmp/fc-snap-db2_11.5.log && { echo "FAIL: kernel rejected a parameter"; exit 1; }

zfs destroy tank/fire/db2_11.5@base 2>/dev/null || true
zfs snapshot tank/fire/db2_11.5@base

cd /
out=$(echo '["select 1 from sysibm.sysdummy1"]' | /mnt/fire/db2_11.5/run.sh) || true
printf '%s\n' "$out"
