echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/postgres_18/run.sh' >> /etc/sudoers
zfs create tank/fire/postgres_18
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/postgres_18/vmlinux.bin

<<'EOF' cat > /mnt/fire/postgres_18/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/postgres_18/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/postgres_18/vsock /mnt/fire/postgres_18/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/postgres_18/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/postgres_18/run.sh

mkdir /mnt/fire/postgres_18/mnt

<<'EODOCKER' cat > /mnt/fire/postgres_18/DOCKERFILE
...
apt-get update
apt-get -y install postgresql-18 postgresql-18-postgis-3 postgresql-18-pgrouting postgresql-plpython3-18
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_postgres_18 - < /mnt/fire/postgres_18/DOCKERFILE
#docker run --rm -ti dummy_postgres_18
dd if=/dev/zero bs=1M count=950 > /mnt/fire/postgres_18/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/postgres_18/rootfs.ext4
mount -o loop /mnt/fire/postgres_18/rootfs.ext4 /mnt/fire/postgres_18/mnt
docker run --rm -ti -v /mnt/fire/postgres_18/mnt:/my-rootfs dummy_postgres_18
...
cat <<"EOF" > /mnt/fire/postgres_18/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/postgres_18/mnt/fiddle.sh

#mount -o loop /mnt/fire/postgres_18/rootfs.ext4 /mnt/fire/postgres_18/mnt
<<'EOC' cat > /mnt/fire/postgres_18/fiddle.c
...
cp /mnt/fire/postgres_18/fiddle.c /mnt/fire/postgres_18/mnt/fiddle.c
docker run --rm -v /mnt/fire/postgres_18/mnt:/my-rootfs dummy_postgres_18 -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends gcc libc6-dev libpq-dev && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/postgres_18/mnt /usr/bin/ldd /fiddle

cp /mnt/fire/postgres_18/vsock /mnt/fire/postgres_18/mnt/vsock
chmod 755 /mnt/fire/postgres_18/mnt/vsock

umount /mnt/fire/postgres_18/mnt

zfs set recordsize=16K tank/fire/postgres_18

cd /mnt/fire/postgres_18
rm -f mem vmstate v.sock* /tmp/fc-snap-postgres_18.sock /tmp/fc-snap-postgres_18.log
firecracker-1.13 --api-sock /tmp/fc-snap-postgres_18.sock --config-file config.json > /tmp/fc-snap-postgres_18.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-postgres_18.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-postgres_18.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-postgres_18.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/postgres_18@base 2>/dev/null || true
zfs snapshot tank/fire/postgres_18@base

out=$(echo '["select 1"]' | /mnt/fire/postgres_18/run.sh) || true
printf '%s\n' "$out"
