echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/mariadb_10.2/run.sh' >> /etc/sudoers
zfs create tank/fire/mariadb_10.2
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/mariadb_10.2/vmlinux.bin

<<'EOF' cat > /mnt/fire/mariadb_10.2/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/mariadb_10.2/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/mariadb_10.2/vsock /mnt/fire/mariadb_10.2/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/mariadb_10.2/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/mariadb_10.2/run.sh

mkdir /mnt/fire/mariadb_10.2/mnt

# ---- which MariaDB, and from where ----
# stretch, not buster: this frozen repo's buster dist serves 10.2.22, not 10.2.44
<<'EODOCKER' cat > /mnt/fire/mariadb_10.2/DOCKERFILE
FROM debian:stretch-slim
# apt-transport-https: stretch's apt has no built-in https method
RUN printf 'deb http://archive.debian.org/debian stretch main\ndeb http://archive.debian.org/debian-security stretch/updates main\n' \
      > /etc/apt/sources.list \
 && echo 'Acquire::Check-Valid-Until "false";' > /etc/apt/apt.conf.d/99archive \
 && apt-get update && apt-get install --no-install-recommends -y \
      systemd-sysv udev haveged ca-certificates curl apt-transport-https
RUN install -d /etc/apt/keyrings \
 ...
 && echo 'deb [signed-by=/etc/apt/keyrings/mariadb.asc] https://archive.mariadb.org/mariadb-10.2.44/repo/debian stretch main' \
      > /etc/apt/sources.list.d/mariadb.list \
 && apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y \
      mariadb-server=10.2.44+maria~stretch \
      mariadb-client=10.2.44+maria~stretch
# the frozen repo carries other patch releases too: fail on any package that is not 10.2.44
RUN dpkg-query -W -f='${Package} ${Version}\n' | grep -E '^(mariadb|libmariadb)' ; \
    bad=$(dpkg-query -W -f='${Package} ${Version}\n' | grep -E '^(mariadb|libmariadb)' \
          | grep -v '10\.2\.44+maria~stretch' || true) ; \
    [ -z "$bad" ] || { echo "FAIL: non-10.2.44 mariadb package installed: $bad" ; exit 1 ; }
RUN mysqld --version
# under docker the postinst leaves no mysql schema, and mysqld dies on the missing mysql.user
...
RUN install -d -o mysql -g mysql /run/mysqld \
 ...
 && mysql -e 'create database fiddle' \
...
# fiddle.sh starts the server itself: any packaged unit left enabled, including one systemd
# generates from /etc/init.d, puts a second server on the datadir and fiddles come back empty
RUN systemctl disable mariadb ; \
    rm -f /etc/init.d/mysql /etc/init.d/mariadb ; \
    rm -f /etc/rc?.d/[SK][0-9][0-9]mysql /etc/rc?.d/[SK][0-9][0-9]mariadb ; \
    left=$(find /etc/systemd/system -path '*.wants/*' -printf '%f\n' 2>/dev/null \
           | grep -iE 'maria|mysql' || true) ; \
    [ -z "$left" ] || { echo "FAIL: packaged server unit still enabled: $left" ; exit 1 ; } ; \
    echo "OK: no packaged server unit is enabled"
...
# to every copy
<<'EOC' cat > /mnt/fire/mariadb_10.2/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <mysql.h>
...
int main(int argc, char **argv){
  ...
  /* This release seeds a session's RAND() from the THD's address and global_query_id
     alone, which a restored guest repeats: reseed it. /dev/urandom, because stretch's
     glibc 2.24 has no getrandom(). */
  unsigned a = 0, b = 0;
  FILE *ur = fopen("/dev/urandom", "rb");
  if(ur){
    int got = (fread(&a, sizeof a, 1, ur) == 1) && (fread(&b, sizeof b, 1, ur) == 1);
    fclose(ur);
    if(got){
      char seedq[80];
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_mariadb_10.2 - < /mnt/fire/mariadb_10.2/DOCKERFILE

# re-derive this count per engine; never copy a sibling's
dd if=/dev/zero bs=1M count=540 > /mnt/fire/mariadb_10.2/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/mariadb_10.2/rootfs.ext4
mount -o loop /mnt/fire/mariadb_10.2/rootfs.ext4 /mnt/fire/mariadb_10.2/mnt
docker run --rm -ti -v /mnt/fire/mariadb_10.2/mnt:/my-rootfs dummy_mariadb_10.2
...
# libssl-dev: 10.2's mysql_config --libs adds -lssl -lcrypto
cp /mnt/fire/mariadb_10.2/fiddle.c /mnt/fire/mariadb_10.2/mnt/fiddle.c
docker run --rm -v /mnt/fire/mariadb_10.2/mnt:/my-rootfs debian:stretch-slim bash -c '
  set -e
  printf "deb http://archive.debian.org/debian stretch main\ndeb http://archive.debian.org/debian-security stretch/updates main\n" > /etc/apt/sources.list
  echo "Acquire::Check-Valid-Until \"false\";" > /etc/apt/apt.conf.d/99archive
  apt-get update -qq && apt-get install -y -qq --no-install-recommends ca-certificates curl apt-transport-https
  install -d /etc/apt/keyrings
  curl -sfLo /etc/apt/keyrings/mariadb.asc https://archive.mariadb.org/PublicKey
  echo "00adb387f4c12e8982cfc28165d225695621494354cd58108c6845063bfb74b5  /etc/apt/keyrings/mariadb.asc" | sha256sum -c -
  echo "deb [signed-by=/etc/apt/keyrings/mariadb.asc] https://archive.mariadb.org/mariadb-10.2.44/repo/debian stretch main" > /etc/apt/sources.list.d/mariadb.list
  apt-get update -qq
  apt-get install -y -qq --no-install-recommends gcc libc6-dev libssl-dev libmariadb-dev=10.2.44+maria~stretch
  gcc -O2 -Wall -o /my-rootfs/fiddle /my-rootfs/fiddle.c $(mysql_config --cflags --libs)
  rm /my-rootfs/fiddle.c'
chroot /mnt/fire/mariadb_10.2/mnt /usr/bin/ldd /fiddle

docker rmi dummy_mariadb_10.2

cp /mnt/fire/mariadb_10.2/vsock /mnt/fire/mariadb_10.2/mnt/vsock
chmod 755 /mnt/fire/mariadb_10.2/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/mariadb_10.2/mnt/fiddle.sh
#!/bin/sh
...
/usr/sbin/mysqld --user=root --skip-networking > /var/log/mysqld-boot.log 2>&1 &
...
echo "mysqld: $(tail -2 /var/log/mysqld-boot.log)" > /dev/console
...
chmod 700 /mnt/fire/mariadb_10.2/mnt/fiddle.sh

umount /mnt/fire/mariadb_10.2/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/mariadb_10.2/rootfs.ext4 2>/dev/null | grep -E 'Block (count|size)|Free blocks'
blk=$(dumpe2fs -h /mnt/fire/mariadb_10.2/rootfs.ext4 2>/dev/null | awk '/^Block size:/{print $3}')
free_mb=$(( $(dumpe2fs -h /mnt/fire/mariadb_10.2/rootfs.ext4 2>/dev/null | awk '/^Free blocks:/{print $3}') * blk / 1048576 ))
...
zfs set recordsize=16K tank/fire/mariadb_10.2

cd /mnt/fire/mariadb_10.2
rm -f mem vmstate v.sock* /tmp/fc-snap-mariadb_10.2.sock /tmp/fc-snap-mariadb_10.2.log
firecracker-1.13 --api-sock /tmp/fc-snap-mariadb_10.2.sock --config-file config.json > /tmp/fc-snap-mariadb_10.2.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-mariadb_10.2.log ; do sleep 0.1 ; done
# must show a result, or the guest snapshotted with no reachable server
grep -a 'warm-up:' /tmp/fc-snap-mariadb_10.2.log
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-mariadb_10.2.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-mariadb_10.2.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/mariadb_10.2@base 2>/dev/null || true
zfs snapshot tank/fire/mariadb_10.2@base

out=$(echo '["select version()"]' | /mnt/fire/mariadb_10.2/run.sh) || true
printf '%s\n' "$out"
