echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/timescaledb_2.28/run.sh' >> /etc/sudoers
zfs create tank/fire/timescaledb_2.28
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/timescaledb_2.28/vmlinux.bin

<<'EOF' cat > /mnt/fire/timescaledb_2.28/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/timescaledb_2.28/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/timescaledb_2.28/vsock /mnt/fire/timescaledb_2.28/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/timescaledb_2.28/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/timescaledb_2.28/run.sh

mkdir /mnt/fire/timescaledb_2.28/mnt

<<'EODOCKER' cat > /mnt/fire/timescaledb_2.28/DOCKERFILE
...
echo "deb https://packagecloud.io/timescale/timescaledb/debian/ trixie main" > /etc/apt/sources.list.d/timescaledb.list
wget --quiet -O /etc/apt/trusted.gpg.d/timescaledb.asc https://packagecloud.io/timescale/timescaledb/gpgkey
apt-get update
apt-get -y install postgresql-18 postgresql-plpython3-18 timescaledb-2-postgresql-18='2.28*' timescaledb-2-loader-postgresql-18='2.28*' timescaledb-toolkit-postgresql-18
...
EOF
# the postgres family's conf plus the preload line: keep it in step with postgres_18's
...
EOF
EORUN
# stop cleanly: a dirty stop leaves the ceremony's boot replaying WAL into the memory image
RUN <<'EORUN'
pg_ctlcluster 18 main start
psql postgres postgres -c 'create extension timescaledb;'
psql postgres postgres -c 'create extension timescaledb_toolkit;'
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_timescaledb_2.28 - < /mnt/fire/timescaledb_2.28/DOCKERFILE
#docker run --rm -ti dummy_timescaledb_2.28
# 981: count=995 made the gate read 89M, and the gate moves 1:1 with count, so 75M free is 981
dd if=/dev/zero bs=1M count=981 > /mnt/fire/timescaledb_2.28/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/timescaledb_2.28/rootfs.ext4
mount -o loop /mnt/fire/timescaledb_2.28/rootfs.ext4 /mnt/fire/timescaledb_2.28/mnt
docker run --rm -ti -v /mnt/fire/timescaledb_2.28/mnt:/my-rootfs dummy_timescaledb_2.28
...
cat <<"EOF" > /mnt/fire/timescaledb_2.28/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/timescaledb_2.28/mnt/fiddle.sh

#mount -o loop /mnt/fire/timescaledb_2.28/rootfs.ext4 /mnt/fire/timescaledb_2.28/mnt
<<'EOC' cat > /mnt/fire/timescaledb_2.28/fiddle.c
...
cp /mnt/fire/timescaledb_2.28/fiddle.c /mnt/fire/timescaledb_2.28/mnt/fiddle.c
docker run --rm -v /mnt/fire/timescaledb_2.28/mnt:/my-rootfs dummy_timescaledb_2.28 -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends gcc libc6-dev libpq-dev && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/timescaledb_2.28/mnt /usr/bin/ldd /fiddle
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_timescaledb_2.28

cp /mnt/fire/timescaledb_2.28/vsock /mnt/fire/timescaledb_2.28/mnt/vsock
chmod 755 /mnt/fire/timescaledb_2.28/mnt/vsock

umount /mnt/fire/timescaledb_2.28/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/timescaledb_2.28/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/timescaledb_2.28

cd /mnt/fire/timescaledb_2.28
# per-engine log and socket: a concurrent ceremony on a shared path steals the snapshot
rm -f mem vmstate v.sock* /tmp/fc-snap-timescaledb_2.28.sock /tmp/fc-snap-timescaledb_2.28.log
firecracker-1.13 --api-sock /tmp/fc-snap-timescaledb_2.28.sock --config-file config.json > /tmp/fc-snap-timescaledb_2.28.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-timescaledb_2.28.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-timescaledb_2.28.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-timescaledb_2.28.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-timescaledb_2.28.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/timescaledb_2.28@base 2>/dev/null || true
zfs snapshot tank/fire/timescaledb_2.28@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/timescaledb_2.28/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/timescaledb_2.28/run.sh) || true
printf '%s\n' "$out"
