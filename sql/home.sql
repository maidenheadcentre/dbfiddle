...
create function get() returns jsonb as $$
  select to_jsonb((
    select z
    from( select coalesce(reltuples::integer,0) source_total_count
               , (select json_agg(z order by engine_total_90 desc)
                  from ( select engine_code
                              ...
                              , (select encode(fiddle_code,'hex')
                                 from fiddle f
                                 where f.engine_code=e.engine_code and
                                       f.version_code=e.engine_default_version_code and 
                                       f.sample_name='' and
                                       f.fiddle_hash=decode(md5(convert_to(to_jsonb(e.engine_default)::text,'utf8')),'hex')) engine_fiddle_code
                         from engine e natural join (select engine_code
                                                          , coalesce(sum(fiddle_daily_count),0)::integer engine_total
                                                          , coalesce((sum(fiddle_daily_count) filter (where fiddle_daily_on<current_date and
                                                                                                   fiddle_daily_on>=current_date-90)),0)::integer engine_total_90
                                                          , coalesce((sum(fiddle_daily_count) filter (where fiddle_daily_on<current_date and
                                                                                                   fiddle_daily_on>=current_date-7)),0)::integer engine_total_7
                                                     from fiddle_daily
                                                     group by engine_code) z) z) engines
               , (select json_agg(z order by name)
                  from ( select engine_name || ' ' || version_name || case when sample_name<>'' then ' ('||sample_name||')' else '' end name
                              , allowed_fail_since is not null is_down
                         from engine e natural join version natural join allowed) z) alloweds
          from pg_class
          where oid = 'public.source'::regclass ) z ));
