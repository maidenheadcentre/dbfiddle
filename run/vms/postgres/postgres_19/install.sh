echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/postgres_19/run.sh' >> /etc/sudoers
zfs create tank/fire/postgres_19
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/postgres_19/vmlinux.bin

<<'EOF' cat > /mnt/fire/postgres_19/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/postgres_19/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/postgres_19/vsock /mnt/fire/postgres_19/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/postgres_19/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/postgres_19/run.sh

mkdir /mnt/fire/postgres_19/mnt

<<'EODOCKER' cat > /mnt/fire/postgres_19/DOCKERFILE
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_postgres_19 - < /mnt/fire/postgres_19/DOCKERFILE
#docker run --rm -ti dummy_postgres_19
# re-derive count= from what the gate after the ceremony prints, never from du in the builder container
dd if=/dev/zero bs=1M count=1145 > /mnt/fire/postgres_19/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/postgres_19/rootfs.ext4
mount -o loop /mnt/fire/postgres_19/rootfs.ext4 /mnt/fire/postgres_19/mnt
docker run --rm -ti -v /mnt/fire/postgres_19/mnt:/my-rootfs dummy_postgres_19
...
cat <<"EOF" > /mnt/fire/postgres_19/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/postgres_19/mnt/fiddle.sh

#mount -o loop /mnt/fire/postgres_19/rootfs.ext4 /mnt/fire/postgres_19/mnt
<<'EOC' cat > /mnt/fire/postgres_19/fiddle.c
...
cp /mnt/fire/postgres_19/fiddle.c /mnt/fire/postgres_19/mnt/fiddle.c
docker run --rm -v /mnt/fire/postgres_19/mnt:/my-rootfs dummy_postgres_19 -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends gcc libc6-dev libpq-dev && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/postgres_19/mnt /usr/bin/ldd /fiddle

cp /mnt/fire/postgres_19/vsock /mnt/fire/postgres_19/mnt/vsock
chmod 755 /mnt/fire/postgres_19/mnt/vsock

umount /mnt/fire/postgres_19/mnt

zfs set recordsize=16K tank/fire/postgres_19

cd /mnt/fire/postgres_19
rm -f mem vmstate v.sock* /tmp/fc-snap-postgres_19.sock /tmp/fc-snap-postgres_19.log
firecracker-1.13 --api-sock /tmp/fc-snap-postgres_19.sock --config-file config.json > /tmp/fc-snap-postgres_19.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-postgres_19.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-postgres_19.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-postgres_19.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/postgres_19@base 2>/dev/null || true
zfs snapshot tank/fire/postgres_19@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/postgres_19/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/postgres_19/run.sh) || true
printf '%s\n' "$out"
...
cat > /tmp/postgres_19-langcheck.json <<'EOF'
...
EOF
lang=$(/mnt/fire/postgres_19/run.sh < /tmp/postgres_19-langcheck.json) || true
printf '%s\n' "$lang"
...
cat > /tmp/postgres_19-crashcheck.json <<'EOF'
["select 1 as before","copy (select 1) to program 'kill -SEGV $PPID'","select 2 as after"]
EOF
crash=$(/mnt/fire/postgres_19/run.sh < /tmp/postgres_19-crashcheck.json) || true
printf '%s\n' "$crash"
