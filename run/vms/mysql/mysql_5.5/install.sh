...
echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/mysql_5.5/run.sh' >> /etc/sudoers
zfs create tank/fire/mysql_5.5
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/mysql_5.5/vmlinux.bin

<<'EOF' cat > /mnt/fire/mysql_5.5/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/mysql_5.5/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/mysql_5.5/vsock /mnt/fire/mysql_5.5/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/mysql_5.5/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/mysql_5.5/run.sh

mkdir /mnt/fire/mysql_5.5/mnt

<<'EOC' cat > /mnt/fire/mysql_5.5/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
cat > /mnt/fire/mysql_5.5/DOCKERFILE <<"EOF"
FROM mysql:5.5
RUN rm -f /etc/apt/sources.list.d/mysql.list
# --allow-unauthenticated: this image's archive keyring predates stretch's key renewal,
# so the Release files fail with EXPKEYSIG
RUN printf 'deb http://archive.debian.org/debian stretch main\ndeb http://archive.debian.org/debian-security stretch/updates main\n' \
      > /etc/apt/sources.list \
 && echo 'Acquire::Check-Valid-Until "false";' > /etc/apt/apt.conf.d/99archive \
 && apt-get update \
 && apt-get install -y --allow-unauthenticated --no-install-recommends systemd-sysv udev haveged libmariadb2
RUN printf '[mysqld]\nsocket = /var/run/mysqld/mysqld.sock\n[client]\nsocket = /var/run/mysqld/mysqld.sock\n' \
      > /etc/mysql/conf.d/fiddle-socket.cnf
RUN mkdir -p /var/run/mysqld \
  && mysql_install_db --user=mysql --ldata=/var/lib/mysql --basedir=/usr/local/mysql \
  && ( /usr/local/mysql/bin/mysqld --user=root --skip-networking & ) \
  && sleep 10 \
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_mysql_5.5 - < /mnt/fire/mysql_5.5/DOCKERFILE
#docker run --rm -ti dummy_mysql_5.5
dd if=/dev/zero bs=1M count=355 > /mnt/fire/mysql_5.5/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/mysql_5.5/rootfs.ext4
mount -o loop /mnt/fire/mysql_5.5/rootfs.ext4 /mnt/fire/mysql_5.5/mnt
cp /mnt/fire/mysql_5.5/fiddle.c /mnt/fire/mysql_5.5/mnt/fiddle.c
# the builder's release must match the guest's, or the runner links against a glibc the guest lacks
docker run --rm -v /mnt/fire/mysql_5.5/mnt:/my-rootfs debian:stretch-slim bash -c '
  set -e
...
docker run --rm -ti -v /mnt/fire/mysql_5.5/mnt:/my-rootfs dummy_mysql_5.5
...
docker rmi dummy_mysql_5.5 || true

cp /mnt/fire/mysql_5.5/vsock /mnt/fire/mysql_5.5/mnt/vsock
chmod 755 /mnt/fire/mysql_5.5/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/mysql_5.5/mnt/fiddle.sh
#!/bin/sh
...
date -u -s @946684800
/usr/local/mysql/bin/mysqld --user=root --skip-networking > /dev/null 2>&1 &
...
chmod 700 /mnt/fire/mysql_5.5/mnt/fiddle.sh

umount /mnt/fire/mysql_5.5/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/mysql_5.5/rootfs.ext4 2>/dev/null | awk '
...
zfs set recordsize=16K tank/fire/mysql_5.5

cd /mnt/fire/mysql_5.5
rm -f mem vmstate v.sock* /tmp/fc-snap-mysql_5.5.sock /tmp/fc-snap-mysql_5.5.log
firecracker-1.13 --api-sock /tmp/fc-snap-mysql_5.5.sock --config-file config.json > /tmp/fc-snap-mysql_5.5.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-mysql_5.5.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-mysql_5.5.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-mysql_5.5.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
...
zfs destroy tank/fire/mysql_5.5@base 2>/dev/null || true
zfs snapshot tank/fire/mysql_5.5@base

# run.sh has no meaningful exit status: assert on the body
out=$(echo '["select version()"]' | /mnt/fire/mysql_5.5/run.sh) || true
printf '%s\n' "$out"
