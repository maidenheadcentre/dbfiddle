# mssql-server comes from the mssql-server-2017 channel, which cannot cross into a later
# release. Never take it from prod.list's unversioned package.
echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/sqlserver_2017l/run.sh' >> /etc/sudoers
zfs create tank/fire/sqlserver_2017l
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/sqlserver_2017l/vmlinux.bin
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/cpu_template.json
{
  "cpuid_modifiers": [
    {
      "leaf": "0x80000007",
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2017l/vsock /mnt/fire/sqlserver_2017l/vsock.c
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/reseedrng.c
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2017l/reseedrng /mnt/fire/sqlserver_2017l/reseedrng.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/sqlserver_2017l/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/sqlserver_2017l/run.sh

mkdir /mnt/fire/sqlserver_2017l/mnt
mkdir /mnt/fire/sqlserver_2017l/build
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/build/fiddle.csproj
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/build/fiddle.cs
using System;
...
<<'EOF' cat > /mnt/fire/sqlserver_2017l/build/Dockerfile
FROM ubuntu:18.04 AS build
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y curl gnupg ca-certificates \
 && curl -fsSL https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor -o /usr/share/keyrings/microsoft-prod.gpg \
 && curl -fsSL https://packages.microsoft.com/config/ubuntu/18.04/prod.list -o /etc/apt/sources.list.d/mssql-prod.list \
 && sed -i 's|\[arch=\([^]]*\)\]|[arch=\1 signed-by=/usr/share/keyrings/microsoft-prod.gpg]|' /etc/apt/sources.list.d/*.list
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y dotnet-sdk-6.0
...
RUN curl -fsSL https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor -o /usr/share/keyrings/microsoft-prod.gpg \
 && curl -fsSL https://packages.microsoft.com/config/ubuntu/18.04/mssql-server-2017.list -o /etc/apt/sources.list.d/mssql-server-2017.list \
 && curl -fsSL https://packages.microsoft.com/config/ubuntu/18.04/prod.list -o /etc/apt/sources.list.d/mssql-prod.list \
 && sed -i 's|\[arch=\([^]]*\)\]|[arch=\1 signed-by=/usr/share/keyrings/microsoft-prod.gpg]|' /etc/apt/sources.list.d/*.list
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y mssql-server dotnet-runtime-6.0 \
...
docker build -t dummy_sqlserver_2017l /mnt/fire/sqlserver_2017l/build
# 1648M = 1507M content + 63M ext4 metadata + 78M free. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=1648 > /mnt/fire/sqlserver_2017l/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/sqlserver_2017l/rootfs.ext4
mount -o loop /mnt/fire/sqlserver_2017l/rootfs.ext4 /mnt/fire/sqlserver_2017l/mnt
docker run --rm -i -v /mnt/fire/sqlserver_2017l/mnt:/my-rootfs dummy_sqlserver_2017l -s <<"SETUP"
...
docker rmi dummy_sqlserver_2017l

cp /mnt/fire/sqlserver_2017l/vsock /mnt/fire/sqlserver_2017l/mnt/vsock
chmod 755 /mnt/fire/sqlserver_2017l/mnt/vsock
cp /mnt/fire/sqlserver_2017l/reseedrng /mnt/fire/sqlserver_2017l/mnt/reseedrng
chmod 755 /mnt/fire/sqlserver_2017l/mnt/reseedrng

# replace docker's bind-mounted network identity with the guest's offline one
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/sqlserver_2017l/mnt/etc/hosts
echo fiddle > /mnt/fire/sqlserver_2017l/mnt/etc/hostname
: > /mnt/fire/sqlserver_2017l/mnt/etc/resolv.conf
rm -f /mnt/fire/sqlserver_2017l/mnt/etc/systemd/system/multi-user.target.wants/mssql-server.service

# ---- init boot ----
<<'EOF' cat > /mnt/fire/sqlserver_2017l/mnt/fiddle.sh
#!/bin/sh
...
reboot -ff
EOF
chmod 700 /mnt/fire/sqlserver_2017l/mnt/fiddle.sh
umount /mnt/fire/sqlserver_2017l/mnt

cd /mnt/fire/sqlserver_2017l
rm -f /tmp/fc-init-sqlserver_2017l.log
firecracker-1.13 --no-api --config-file config.json > /tmp/fc-init-sqlserver_2017l.log 2>&1
grep FIDDLE-INIT-DONE /tmp/fc-init-sqlserver_2017l.log   # must appear; if not, read the log
grep -a 'FIDDLE-DB-RC=0' /tmp/fc-init-sqlserver_2017l.log        # the `fiddle` database was created
grep -a 'FIDDLE-DB-STATE-RC=0' /tmp/fc-init-sqlserver_2017l.log  # ...with snapshot isolation on
...
dumpe2fs -h /mnt/fire/sqlserver_2017l/rootfs.ext4 2>/dev/null | grep -E 'Block count|Free blocks|Block size|Reserved block count'
dumpe2fs -h /mnt/fire/sqlserver_2017l/rootfs.ext4 2>/dev/null | awk -F: '
...
rm -f mem vmstate v.sock* /tmp/fc-snap-sqlserver_2017l.sock /tmp/fc-snap-sqlserver_2017l.log
firecracker-1.13 --api-sock /tmp/fc-snap-sqlserver_2017l.sock --config-file config.json > /tmp/fc-snap-sqlserver_2017l.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-sqlserver_2017l.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2017l.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2017l.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
...
grep -a SQLSERVR-WARM /tmp/fc-snap-sqlserver_2017l.log       # must appear, BEFORE FIDDLE-READY
grep -a 'Kernel command line' /tmp/fc-snap-sqlserver_2017l.log | grep -a no-kvmapf   # must match
grep -a 'Unknown kernel command line parameters' /tmp/fc-snap-sqlserver_2017l.log    # must NOT match
grep -a CLOCK-AGED /tmp/fc-snap-sqlserver_2017l.log          # randomness lever 1 ran
grep -a 'RESEEDRNG: .* states re-keyed' /tmp/fc-snap-sqlserver_2017l.log   # lever 2's layout still matches
grep -a 'RESEEDRNG: .* 0 states re-keyed' /tmp/fc-snap-sqlserver_2017l.log && echo 'CEREMONY FAILED: CNG layout no longer matches'

# leaked clones block the destroy, leaving a stale @base the snapshot line then trips on
zfs destroy tank/fire/sqlserver_2017l@base 2>/dev/null || true
zfs snapshot tank/fire/sqlserver_2017l@base || echo 'CEREMONY FAILED: stale @base still in place (leaked clones?)'

screen_bad=0
for i in $(seq 1 $SCREEN_N); do
  [ -n "$(echo '["select 1"]' | /mnt/fire/sqlserver_2017l/run.sh 2>/dev/null)" ] || screen_bad=$((screen_bad + 1))
done
...
out=$(echo '["select @@version"]' | /mnt/fire/sqlserver_2017l/run.sh) || true
printf '%s\n' "$out"
