echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/yugabytedb_2.6/run.sh' >> /etc/sudoers
zfs create tank/fire/yugabytedb_2.6
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/yugabytedb_2.6/vmlinux.bin

<<'EOF' cat > /mnt/fire/yugabytedb_2.6/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/yugabytedb_2.6/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/yugabytedb_2.6/vsock /mnt/fire/yugabytedb_2.6/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/yugabytedb_2.6/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/yugabytedb_2.6/run.sh

mkdir /mnt/fire/yugabytedb_2.6/mnt

<<'EODOCKER' cat > /mnt/fire/yugabytedb_2.6/DOCKERFILE
FROM yugabytedb/yugabyte:2.6.20.0-b10
RUN sed -i -e 's/^mirrorlist=/#mirrorlist=/' \
           -e 's|^#baseurl=http://mirror.centos.org|baseurl=http://vault.centos.org|' \
           /etc/yum.repos.d/CentOS-Base.repo \
 && yum -y --disablerepo='epel*' install postgresql-libs \
...
docker build --no-cache -t dummy_yugabytedb_2.6 - < /mnt/fire/yugabytedb_2.6/DOCKERFILE
#docker run --rm -ti dummy_yugabytedb_2.6
# 1920 = 1845M populated + 75M free. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=1920 > /mnt/fire/yugabytedb_2.6/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/yugabytedb_2.6/rootfs.ext4
mount -o loop /mnt/fire/yugabytedb_2.6/rootfs.ext4 /mnt/fire/yugabytedb_2.6/mnt
docker run --rm -ti -v /mnt/fire/yugabytedb_2.6/mnt:/my-rootfs dummy_yugabytedb_2.6
...
<<'EOF' cat > /mnt/fire/yugabytedb_2.6/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/yugabytedb_2.6/mnt/fiddle.sh

#mount -o loop /mnt/fire/yugabytedb_2.6/rootfs.ext4 /mnt/fire/yugabytedb_2.6/mnt
<<'EOC' cat > /mnt/fire/yugabytedb_2.6/fiddle.c
...
cp /mnt/fire/yugabytedb_2.6/fiddle.c /mnt/fire/yugabytedb_2.6/mnt/fiddle.c
docker run --rm --entrypoint bash -v /mnt/fire/yugabytedb_2.6/mnt:/my-rootfs dummy_yugabytedb_2.6 -c 'yum -y --disablerepo="epel*" install gcc postgresql-devel >/dev/null 2>&1 && gcc -O2 -Wall -std=gnu99 -I/usr/include -o /my-rootfs/fiddle /my-rootfs/fiddle.c -L/usr/lib64 -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/yugabytedb_2.6/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/yugabytedb_2.6/mnt /usr/bin/ldd /fiddle | grep -q 'not found' \
  && { echo "ABORT: /fiddle has an unresolved shared library"; exit 1; }
chroot /mnt/fire/yugabytedb_2.6/mnt /usr/bin/ldd /fiddle | grep -q 'libpq\.so\.5 => /' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_yugabytedb_2.6

cp /mnt/fire/yugabytedb_2.6/vsock /mnt/fire/yugabytedb_2.6/mnt/vsock
chmod 755 /mnt/fire/yugabytedb_2.6/mnt/vsock

# fallocate takes one filename: \; not +, which silently punches nothing
find /mnt/fire/yugabytedb_2.6/mnt/home/yugabyte/data -type f -size +1M -exec fallocate -d {} \; 2>/dev/null || true

umount /mnt/fire/yugabytedb_2.6/mnt

dumpe2fs -h /mnt/fire/yugabytedb_2.6/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/yugabytedb_2.6

cd /mnt/fire/yugabytedb_2.6
rm -f mem vmstate v.sock* /tmp/fc-snap-yugabytedb_2.6.sock /tmp/fc-snap-yugabytedb_2.6.log
firecracker-1.13 --api-sock /tmp/fc-snap-yugabytedb_2.6.sock --config-file config.json > /tmp/fc-snap-yugabytedb_2.6.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-yugabytedb_2.6.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2.6.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2.6.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-yugabytedb_2.6.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/yugabytedb_2.6@base 2>/dev/null || true
zfs snapshot tank/fire/yugabytedb_2.6@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/yugabytedb_2.6/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/yugabytedb_2.6/run.sh) || true
printf '%s\n' "$out"
