echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/yugabytedb_2.8/run.sh' >> /etc/sudoers
zfs create tank/fire/yugabytedb_2.8
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/yugabytedb_2.8/vmlinux.bin

<<'EOF' cat > /mnt/fire/yugabytedb_2.8/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/yugabytedb_2.8/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/yugabytedb_2.8/vsock /mnt/fire/yugabytedb_2.8/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/yugabytedb_2.8/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/yugabytedb_2.8/run.sh

mkdir /mnt/fire/yugabytedb_2.8/mnt

<<'EODOCKER' cat > /mnt/fire/yugabytedb_2.8/DOCKERFILE
FROM yugabytedb/yugabyte:2.8.12.0-b5
RUN curl -fsSL -o /etc/pki/rpm-gpg/RPM-GPG-KEY-AlmaLinux-current \
      https://repo.almalinux.org/almalinux/RPM-GPG-KEY-AlmaLinux \
 && rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY-AlmaLinux-current \
 && yum -y --disablerepo='epel*' install libpq \
...
RUN set -eux; \
    mkdir -p /home/yugabyte/data; \
    /home/yugabyte/bin/yb-master --fs_data_dirs=/home/yugabyte/data \
      --rpc_bind_addresses=127.0.0.1 --master_addresses=127.0.0.1:7100 \
      --replication_factor=1 --default_memory_limit_to_ram_ratio=0.30 \
      --durable_wal_write=false --webserver_interface=127.0.0.1 >/tmp/m.log 2>&1 & \
    m=$!; \
    /home/yugabyte/bin/yb-tserver --fs_data_dirs=/home/yugabyte/data \
      --rpc_bind_addresses=127.0.0.1 --tserver_master_addrs=127.0.0.1:7100 \
      --default_memory_limit_to_ram_ratio=0.30 --webserver_interface=127.0.0.1 \
      --pgsql_proxy_bind_address=127.0.0.1:5433 >/tmp/t.log 2>&1 & \
    t=$!; \
    i=0; while [ $i -lt 180 ]; do \
      /home/yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433 >/dev/null 2>&1 && break; \
      i=$((i+1)); sleep 1; done; \
    /home/yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433; \
    kill -TERM $t; wait $t || true; \
    kill -TERM $m; wait $m || true; \
    sync; \
    rm -f /tmp/m.log /tmp/t.log
ENTRYPOINT ["bash"]
EODOCKER

# per-engine tag: concurrent builds on cumbria2 must never share one
docker build --no-cache -t dummy_yugabytedb_2.8 - < /mnt/fire/yugabytedb_2.8/DOCKERFILE
#docker run --rm -ti dummy_yugabytedb_2.8
# 1970 = 1895M populated + 75M free. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=1970 > /mnt/fire/yugabytedb_2.8/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/yugabytedb_2.8/rootfs.ext4
mount -o loop /mnt/fire/yugabytedb_2.8/rootfs.ext4 /mnt/fire/yugabytedb_2.8/mnt
docker run --rm -ti -v /mnt/fire/yugabytedb_2.8/mnt:/my-rootfs dummy_yugabytedb_2.8
...
find /home/yugabyte/data -type f -size +1M -exec fallocate -d {} \;
for d in bin etc home lib lib64 opt root sbin usr dev; do tar cS "/$d" | tar x -C /my-rootfs; done
mkdir /my-rootfs/tmp
...
<<'EOF' cat > /mnt/fire/yugabytedb_2.8/mnt/fiddle.sh
#!/bin/sh
...
/home/yugabyte/bin/yb-master --fs_data_dirs=/home/yugabyte/data \
  --rpc_bind_addresses=127.0.0.1 --master_addresses=127.0.0.1:7100 \
  --replication_factor=1 --default_memory_limit_to_ram_ratio=0.30 \
  --durable_wal_write=false --webserver_interface=127.0.0.1 &
/home/yugabyte/bin/yb-tserver --fs_data_dirs=/home/yugabyte/data \
  --rpc_bind_addresses=127.0.0.1 --tserver_master_addrs=127.0.0.1:7100 \
  --default_memory_limit_to_ram_ratio=0.30 --webserver_interface=127.0.0.1 \
  --pgsql_proxy_bind_address=127.0.0.1:5433 &
until /home/yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433 >/dev/null 2>&1 ; do sleep 0.1 ; done
...
until /home/yugabyte/postgres/bin/pg_isready -h 127.0.0.1 -p 5433 >/dev/null 2>&1 ; do sleep 0.05 ; done
/fiddle
/vsock reply < /data/output.json
reboot -ff
EOF

chmod 700 /mnt/fire/yugabytedb_2.8/mnt/fiddle.sh

#mount -o loop /mnt/fire/yugabytedb_2.8/rootfs.ext4 /mnt/fire/yugabytedb_2.8/mnt
<<'EOC' cat > /mnt/fire/yugabytedb_2.8/fiddle.c
 ...
 * it back to UTF-8.
 */
#define YB_CONNINFO "host=localhost port=5433 user=yugabyte"
...
cp /mnt/fire/yugabytedb_2.8/fiddle.c /mnt/fire/yugabytedb_2.8/mnt/fiddle.c
docker run --rm --entrypoint bash -v /mnt/fire/yugabytedb_2.8/mnt:/my-rootfs dummy_yugabytedb_2.8 -c 'yum -y --disablerepo="epel*" install gcc libpq-devel >/dev/null 2>&1 && gcc -O2 -Wall -I/usr/include -o /my-rootfs/fiddle /my-rootfs/fiddle.c -L/usr/lib64 -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/yugabytedb_2.8/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/yugabytedb_2.8/mnt /usr/bin/ldd /fiddle | grep -q 'not found' \
  && { echo "ABORT: /fiddle has an unresolved shared library"; exit 1; }
chroot /mnt/fire/yugabytedb_2.8/mnt /usr/bin/ldd /fiddle | grep -q 'libpq\.so\.5 => /' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_yugabytedb_2.8

cp /mnt/fire/yugabytedb_2.8/vsock /mnt/fire/yugabytedb_2.8/mnt/vsock
chmod 755 /mnt/fire/yugabytedb_2.8/mnt/vsock

# fallocate takes one filename: \; not +, which silently punches nothing
find /mnt/fire/yugabytedb_2.8/mnt/home/yugabyte/data -type f -size +1M -exec fallocate -d {} \; 2>/dev/null || true

umount /mnt/fire/yugabytedb_2.8/mnt

dumpe2fs -h /mnt/fire/yugabytedb_2.8/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/yugabytedb_2.8

cd /mnt/fire/yugabytedb_2.8
rm -f mem vmstate v.sock* /tmp/fc-snap-yugabytedb_2.8.sock /tmp/fc-snap-yugabytedb_2.8.log
firecracker-1.13 --api-sock /tmp/fc-snap-yugabytedb_2.8.sock --config-file config.json > /tmp/fc-snap-yugabytedb_2.8.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-yugabytedb_2.8.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2.8.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2.8.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-yugabytedb_2.8.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/yugabytedb_2.8@base 2>/dev/null || true
zfs snapshot tank/fire/yugabytedb_2.8@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/yugabytedb_2.8/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/yugabytedb_2.8/run.sh) || true
printf '%s\n' "$out"
