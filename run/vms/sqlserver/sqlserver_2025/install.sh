echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/sqlserver_2025/run.sh' >> /etc/sudoers
zfs create tank/fire/sqlserver_2025
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/sqlserver_2025/vmlinux.bin
...
<<'EOF' cat > /mnt/fire/sqlserver_2025/cpu_template.json
{
  "cpuid_modifiers": [
    {
      "leaf": "0x80000007",
...
<<'EOF' cat > /mnt/fire/sqlserver_2025/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/sqlserver_2025/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2025/vsock /mnt/fire/sqlserver_2025/vsock.c
...
<<'EOF' cat > /mnt/fire/sqlserver_2025/reseedrng.c
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2025/reseedrng /mnt/fire/sqlserver_2025/reseedrng.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/sqlserver_2025/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/sqlserver_2025/run.sh

mkdir /mnt/fire/sqlserver_2025/mnt
mkdir /mnt/fire/sqlserver_2025/build
...
<<'EOF' cat > /mnt/fire/sqlserver_2025/build/fiddle.csproj
...
<<'EOF' cat > /mnt/fire/sqlserver_2025/build/fiddle.cs
using System;
...
class Fiddle
{
    // sa's password is `fiddle` from the init boot on; setup's long one no longer works
    const string SaConnection = "Initial Catalog=master;Server=localhost;Encrypt=False;User Id=sa;Password=fiddle";
    // Type System Version=SQL Server 2008 decides how several types reach the reader: it
    // changes results, not just the connection.
    const string FiddleConnection = "Initial Catalog=fiddle;Server=localhost;Encrypt=False;Type System Version=SQL Server 2008;Pooling=False;User Id=sa;Password=fiddle";
    ...
    static void Main(string[] args)
    {
        ...
        var queries = new List<(string text, string lang)>();
        foreach (var e in JsonSerializer.Deserialize<List<JsonElement>>(File.ReadAllText("/tmp/batches.json")))
        {
            if (e.ValueKind == JsonValueKind.String)
                queries.Add((e.GetString(), ""));
            else if (e.ValueKind == JsonValueKind.Array && e.GetArrayLength() == 2 && e[0].ValueKind == JsonValueKind.String && e[1].ValueKind == JsonValueKind.String)
                queries.Add((e[0].GetString(), e[1].GetString()));
            else
        ...
        foreach (var (query, lang) in queries)
        {
            var markdown = new StringBuilder();
            if (lang != "")
            {
                ret.Add(LangBatch(query, lang, ret.Count, deadline));
                continue;
            }
    ...
    static string LangBatch(string text, string lang, int idx, long deadline)
    {
        ...
        try
        {
            bool killed = RunLangBatch(text, idx, interp, deadline, out string so, out string se, out int status);
            if (so != "") markdown.Append(FencePlain(so));
            if (se != "") markdown.Append(Backtick(se.TrimEnd('\n'), "error"));
            if (killed) markdown.Append(Backtick("batch did not finish within the runner's time budget", "error"));
            else if (status != 0 && se == "") markdown.Append(Backtick("exited with status " + status, "error"));
        }
        catch (Exception ex)
        {
            markdown.Append(Backtick(ex.Message + "\n" + ex.StackTrace, "error"));
    ...
    static bool RunLangBatch(string text, int idx, string interp, long deadline, out string so, out string se, out int status)
    {
        ...
        var tasks = new[] { Capture(p.StandardOutput.BaseStream, oms), Capture(p.StandardError.BaseStream, ems) };
        // wait for EOF on both pipes, not for the exit: a descendant the batch left behind
        // holds them open past its parent
        bool killed = !Task.WaitAll(tasks, (int)Math.Max(0, deadline - Environment.TickCount64));
        if (killed)
        {
        ...
        p.WaitForExit();
        lock (oms) so = Encoding.UTF8.GetString(oms.ToArray());
        lock (ems) se = Encoding.UTF8.GetString(ems.ToArray());
        status = p.ExitCode;
        return killed;
    }

    // one thread per pipe: a child that fills both would deadlock a sequential read
    static Task Capture(Stream s, MemoryStream ms) => Task.Run(() =>
    {
        var buf = new byte[65536];
        int k;
        while ((k = s.Read(buf, 0, buf.Length)) > 0)
            lock (ms) if (ms.Length < Cap) ms.Write(buf, 0, (int)Math.Min(k, Cap - ms.Length));
    });
}
EOF

<<'EOF' cat > /mnt/fire/sqlserver_2025/build/Dockerfile
...
SETUP
docker rmi dummy_sqlserver_2025

cp /mnt/fire/sqlserver_2025/vsock /mnt/fire/sqlserver_2025/mnt/vsock
chmod 755 /mnt/fire/sqlserver_2025/mnt/vsock
cp /mnt/fire/sqlserver_2025/reseedrng /mnt/fire/sqlserver_2025/mnt/reseedrng
chmod 755 /mnt/fire/sqlserver_2025/mnt/reseedrng
...
echo fiddle > /mnt/fire/sqlserver_2025/mnt/etc/hostname
: > /mnt/fire/sqlserver_2025/mnt/etc/resolv.conf
rm -f /mnt/fire/sqlserver_2025/mnt/etc/systemd/system/multi-user.target.wants/mssql-server.service

# ---- init boot ----
<<'EOF' cat > /mnt/fire/sqlserver_2025/mnt/fiddle.sh
#!/bin/sh
...
/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P Fiddle2b3E4f5A6b7c8D -C -b -d fiddle \
  -Q "alter database current set allow_snapshot_isolation on; create master key encryption by password = 'Password1';" > /dev/console 2>&1
echo "FIDDLE-DB-STATE-RC=$?" > /dev/console
# sa's password becomes `fiddle`, short enough for a python batch's own connection string.
# The policy refuses it, hence check_policy off; the rest of the family keeps the long one.
/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P Fiddle2b3E4f5A6b7c8D -C -b -Q "alter login sa with password = 'fiddle', check_policy = off" > /dev/console 2>&1
echo "FIDDLE-SA-RC=$?" > /dev/console
sync
echo FIDDLE-INIT-DONE > /dev/console
reboot -ff
EOF
chmod 700 /mnt/fire/sqlserver_2025/mnt/fiddle.sh
umount /mnt/fire/sqlserver_2025/mnt

cd /mnt/fire/sqlserver_2025
rm -f /tmp/fc-init.log
firecracker-1.13 --no-api --config-file config.json > /tmp/fc-init.log 2>&1
grep FIDDLE-INIT-DONE /tmp/fc-init.log   # must appear; if not, read the log
grep -a 'FIDDLE-DB-RC=0' /tmp/fc-init.log        # the `fiddle` database was created
grep -a 'FIDDLE-DB-STATE-RC=0' /tmp/fc-init.log  # ...with snapshot isolation and a master key
# without the alter the ceremony's ping never succeeds and the build hangs: stop here instead
grep -a 'FIDDLE-SA-RC=0' /tmp/fc-init.log || { echo "ABORT: sa's password was not changed to fiddle"; exit 1; }
...
cd /mnt/fire/sqlserver_2025
rm -f mem vmstate v.sock* /tmp/fc-snap-sqlserver_2025.sock /tmp/fc-snap-sqlserver_2025.log
firecracker-1.13 --api-sock /tmp/fc-snap-sqlserver_2025.sock --config-file config.json > /tmp/fc-snap-sqlserver_2025.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-sqlserver_2025.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2025.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2025.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
# a snapshot of a cold guest just serves slowly: assert it is warm and the boot arg took
grep -a SQLSERVR-WARM /tmp/fc-snap-sqlserver_2025.log       # must appear, BEFORE FIDDLE-READY
...
zfs destroy tank/fire/sqlserver_2025@base 2>/dev/null || true
zfs snapshot tank/fire/sqlserver_2025@base || echo 'CEREMONY FAILED: stale @base still in place (leaked clones?)'

screen_bad=0
for i in $(seq 1 $SCREEN_N); do
  [ -n "$(echo '["select 1"]' | /mnt/fire/sqlserver_2025/run.sh 2>/dev/null)" ] || screen_bad=$((screen_bad + 1))
done
...
gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/sqlserver_2025/rootfs.ext4 "$gate"; then
...
out=$(echo '["select @@version"]' | /mnt/fire/sqlserver_2025/run.sh) || true
printf '%s\n' "$out"
...
cat > /tmp/sqlserver_2025-langcheck.json <<'JSON'
["select 42 x into t",
 ...
 ["import sys\nsys.exit(3)","python"]]
JSON
lang=$(/mnt/fire/sqlserver_2025/run.sh < /tmp/sqlserver_2025-langcheck.json) || true
printf '%s\n' "$lang"
case $lang in *py-sees:42*) ;;
  *) echo "ABORT: the python batch did not read the engine"; exit 1 ;;
esac
case $lang in *"| 1 |"*"| 42 |"*) ;;
  *) echo "ABORT: a committed python insert was not visible to the next SQL batch"; exit 1 ;;
esac
case $lang in *"| 2 |"*) echo "ABORT: an uncommitted python insert leaked into the next SQL batch"; exit 1 ;;
esac
