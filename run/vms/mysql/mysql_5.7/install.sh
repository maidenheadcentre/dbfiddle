...
echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/mysql_5.7/run.sh' >> /etc/sudoers
zfs create tank/fire/mysql_5.7
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/mysql_5.7/vmlinux.bin

<<'EOF' cat > /mnt/fire/mysql_5.7/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/mysql_5.7/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/mysql_5.7/vsock /mnt/fire/mysql_5.7/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/mysql_5.7/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/mysql_5.7/run.sh

mkdir /mnt/fire/mysql_5.7/mnt

<<'EOC' cat > /mnt/fire/mysql_5.7/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
cat > /mnt/fire/mysql_5.7/DOCKERFILE <<"EOF"
...
RUN systemctl mask mysql.service mysql@.service
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_mysql_5.7 - < /mnt/fire/mysql_5.7/DOCKERFILE
#docker run --rm -ti dummy_mysql_5.7
dd if=/dev/zero bs=1M count=725 > /mnt/fire/mysql_5.7/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/mysql_5.7/rootfs.ext4
mount -o loop /mnt/fire/mysql_5.7/rootfs.ext4 /mnt/fire/mysql_5.7/mnt
cp /mnt/fire/mysql_5.7/fiddle.c /mnt/fire/mysql_5.7/mnt/fiddle.c
# the builder's release must match the guest's, or the runner links against a glibc the guest lacks
docker run --rm -v /mnt/fire/mysql_5.7/mnt:/my-rootfs debian:buster-slim bash -c '
  set -e
...
docker run --rm -ti -v /mnt/fire/mysql_5.7/mnt:/my-rootfs dummy_mysql_5.7
...
docker rmi dummy_mysql_5.7 || true

cp /mnt/fire/mysql_5.7/vsock /mnt/fire/mysql_5.7/mnt/vsock
chmod 755 /mnt/fire/mysql_5.7/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/mysql_5.7/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/mysql_5.7/mnt/fiddle.sh

umount /mnt/fire/mysql_5.7/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/mysql_5.7/rootfs.ext4 2>/dev/null | awk '
...
zfs set recordsize=16K tank/fire/mysql_5.7

cd /mnt/fire/mysql_5.7
rm -f mem vmstate v.sock* /tmp/fc-snap-mysql_5.7.sock /tmp/fc-snap-mysql_5.7.log
firecracker-1.13 --api-sock /tmp/fc-snap-mysql_5.7.sock --config-file config.json > /tmp/fc-snap-mysql_5.7.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-mysql_5.7.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-mysql_5.7.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-mysql_5.7.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
...
zfs destroy tank/fire/mysql_5.7@base 2>/dev/null || true
zfs snapshot tank/fire/mysql_5.7@base

# run.sh has no meaningful exit status: assert on the body
out=$(echo '["select version()"]' | /mnt/fire/mysql_5.7/run.sh) || true
printf '%s\n' "$out"
