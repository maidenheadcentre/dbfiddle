echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/yugabytedb_2.18/run.sh' >> /etc/sudoers
zfs create tank/fire/yugabytedb_2.18
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/yugabytedb_2.18/vmlinux.bin

<<'EOF' cat > /mnt/fire/yugabytedb_2.18/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/yugabytedb_2.18/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/yugabytedb_2.18/vsock /mnt/fire/yugabytedb_2.18/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/yugabytedb_2.18/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/yugabytedb_2.18/run.sh

mkdir /mnt/fire/yugabytedb_2.18/mnt

<<'EODOCKER' cat > /mnt/fire/yugabytedb_2.18/DOCKERFILE
FROM debian:bookworm-slim
RUN apt-get update -y \
  && apt-get install -y --no-install-recommends \
       systemd-sysv udev wget ca-certificates python3 python-is-python3 \
       libpq5 procps netbase \
  && rm -rf /var/lib/apt/lists/*

RUN wget -qO - https://downloads.yugabyte.com/releases/2.18.9.0/yugabyte-2.18.9.0-b17-linux-x86_64.tar.gz \
      | tar -zx \
  && mv yugabyte-2.18.9.0 yugabyte
WORKDIR /yugabyte
...
RUN set -eux; \
    ./bin/yugabyted start --master_flags="durable_wal_write=false" --listen 127.0.0.1; \
...
docker build --no-cache -t dummy_yugabytedb_2.18 - < /mnt/fire/yugabytedb_2.18/DOCKERFILE
#docker run --rm -ti dummy_yugabytedb_2.18
# 1547 = 1468M populated + 79M free. Re-derive it; never copy a sibling's.
dd if=/dev/zero bs=1M count=1547 > /mnt/fire/yugabytedb_2.18/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/yugabytedb_2.18/rootfs.ext4
mount -o loop /mnt/fire/yugabytedb_2.18/rootfs.ext4 /mnt/fire/yugabytedb_2.18/mnt
docker run --rm -ti -v /mnt/fire/yugabytedb_2.18/mnt:/my-rootfs dummy_yugabytedb_2.18
...
<<'EOF' cat > /mnt/fire/yugabytedb_2.18/mnt/fiddle.sh
#!/bin/sh
dd if=/dev/random count=1 bs=1
/yugabyte/bin/yugabyted start --master_flags="durable_wal_write=false" --listen 127.0.0.1
...
chmod 700 /mnt/fire/yugabytedb_2.18/mnt/fiddle.sh

#mount -o loop /mnt/fire/yugabytedb_2.18/rootfs.ext4 /mnt/fire/yugabytedb_2.18/mnt
<<'EOC' cat > /mnt/fire/yugabytedb_2.18/fiddle.c
...
cp /mnt/fire/yugabytedb_2.18/fiddle.c /mnt/fire/yugabytedb_2.18/mnt/fiddle.c
docker run --rm --entrypoint bash -v /mnt/fire/yugabytedb_2.18/mnt:/my-rootfs dummy_yugabytedb_2.18 -c 'apt-get update -qq >/dev/null 2>&1 && apt-get install -y -qq gcc libpq-dev >/dev/null 2>&1 && gcc -O2 -Wall -I/usr/include/postgresql -o /my-rootfs/fiddle /my-rootfs/fiddle.c -L/usr/lib/x86_64-linux-gnu -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/yugabytedb_2.18/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/yugabytedb_2.18/mnt /usr/bin/ldd /fiddle | grep -q 'not found' \
  && { echo "ABORT: /fiddle has an unresolved shared library"; exit 1; }
chroot /mnt/fire/yugabytedb_2.18/mnt /usr/bin/ldd /fiddle | grep -q 'libpq\.so\.5 => /' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_yugabytedb_2.18

cp /mnt/fire/yugabytedb_2.18/vsock /mnt/fire/yugabytedb_2.18/mnt/vsock
chmod 755 /mnt/fire/yugabytedb_2.18/mnt/vsock

# fallocate takes one filename: \; not +, which silently punches nothing
find /mnt/fire/yugabytedb_2.18/mnt/root/var -type f -size +1M -exec fallocate -d {} \; 2>/dev/null || true

umount /mnt/fire/yugabytedb_2.18/mnt

dumpe2fs -h /mnt/fire/yugabytedb_2.18/rootfs.ext4 2>/dev/null | awk '
  /^Block size:/ {bs=$3} /^Free blocks:/ {fb=$3}
  END { printf "rootfs headroom before the ceremony: %.1fM free (block size %d)\n", fb*bs/1048576, bs }'

zfs set recordsize=16K tank/fire/yugabytedb_2.18

cd /mnt/fire/yugabytedb_2.18
rm -f mem vmstate v.sock* /tmp/fc-snap-yugabytedb_2.18.sock /tmp/fc-snap-yugabytedb_2.18.log
firecracker-1.13 --api-sock /tmp/fc-snap-yugabytedb_2.18.sock --config-file config.json > /tmp/fc-snap-yugabytedb_2.18.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-yugabytedb_2.18.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2.18.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-yugabytedb_2.18.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
grep -a 'warm-up:' /tmp/fc-snap-yugabytedb_2.18.log || echo "WARNING: no warm-up line in the ceremony log"

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/yugabytedb_2.18@base 2>/dev/null || true
zfs snapshot tank/fire/yugabytedb_2.18@base

gate=$(mktemp -d)
if ! mount -o loop,ro,norecovery /mnt/fire/yugabytedb_2.18/rootfs.ext4 "$gate"; then
...
out=$(echo '["select 1"]' | /mnt/fire/yugabytedb_2.18/run.sh) || true
printf '%s\n' "$out"
