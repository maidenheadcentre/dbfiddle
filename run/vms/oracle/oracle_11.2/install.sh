echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/oracle_11.2/run.sh' >> /etc/sudoers
zfs create tank/fire/oracle_11.2
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/oracle_11.2/vmlinux.bin

# no-kvmapf: a task asleep on an async page fault at snapshot time never wakes after restore
<<'EOF' cat > /mnt/fire/oracle_11.2/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/oracle_11.2/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/oracle_11.2/vsock /mnt/fire/oracle_11.2/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/oracle_11.2/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/oracle_11.2/run.sh

mkdir /mnt/fire/oracle_11.2/mnt

cat > /mnt/fire/oracle_11.2/DOCKERFILE <<"EOF"
FROM oraclelinux:8
# bc and net-tools are the RPM's (%pre, configure); libnsl is libclntsh.so.11.1's at runtime
RUN dnf -y install systemd libaio bc libnsl file net-tools cronie tmux util-linux perl-interpreter hostname
...
RUN --mount=type=bind,source=oracle-xe-11.2.0-1.0.x86_64.rpm,target=/tmp/oracle-xe.rpm \
    echo '6629c8f014402fbc9db844421a6a0d2c71580838f4ac0e8df6659b62bb905268  /tmp/oracle-xe.rpm' | sha256sum -c - \
 && rpm -ivh /tmp/oracle-xe.rpm
RUN echo /u01/app/oracle/product/11.2.0/xe/lib > /etc/ld.so.conf.d/oracle.conf && ldconfig
...
# the RPM has no download source: /iso/oracle11xe.iso holds it and nothing else
mkdir -p /mnt/oracle11xe
mountpoint -q /mnt/oracle11xe || mount -o loop,ro /iso/oracle11xe.iso /mnt/oracle11xe
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_oracle_11.2 -f /mnt/fire/oracle_11.2/DOCKERFILE /mnt/oracle11xe
dd if=/dev/zero bs=1M count=18432 > /mnt/fire/oracle_11.2/rootfs.ext4
mkfs.ext4 /mnt/fire/oracle_11.2/rootfs.ext4
mount -o loop /mnt/fire/oracle_11.2/rootfs.ext4 /mnt/fire/oracle_11.2/mnt
# --hostname is baked into listener.ora and tnsnames.ora: it must match the guest's /etc/hosts
docker run --rm -ti --hostname fiddle --shm-size 2g -v /mnt/fire/oracle_11.2/mnt:/my-rootfs dummy_oracle_11.2
# configure's prompts: APEX HTTP port, listener port, password, confirmation, start-on-boot
(echo 8080; echo 1521; echo wXYyXLlDVZB0ss2F; echo wXYyXLlDVZB0ss2F; echo n) | /etc/init.d/oracle-xe configure
export ORACLE_HOME=/u01/app/oracle/product/11.2.0/xe ORACLE_SID=XE
...
exit
SQL
$ORACLE_HOME/bin/sqlplus fiddle/IIGjTTbsEzh64McU@//localhost:1521/xe <<"SQL"
...
for d in bin etc home lib lib64 opt root sbin usr u01 dev run var; do tar c "/$d" | tar x -C /my-rootfs; done
...
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/oracle_11.2/mnt/etc/hosts
echo fiddle > /mnt/fire/oracle_11.2/mnt/etc/hostname
# the sysv-generator would start the instance alongside fiddle.sh from these links; the ls
# makes an RPM that stops creating them fail here instead of leaving the rm a no-op
ls /mnt/fire/oracle_11.2/mnt/etc/rc.d/rc?.d/S80oracle-xe >/dev/null
rm -f /mnt/fire/oracle_11.2/mnt/etc/rc.d/rc?.d/S80oracle-xe

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/oracle_11.2/mnt/fiddle.sh
#!/bin/sh
...
until echo "select open_mode from v\$database;" | runuser -u oracle -- $ORACLE_HOME/bin/sqlplus -s / as sysdba | grep -q 'READ WRITE' ; do sleep 0.5 ; done
...
chmod 700 /mnt/fire/oracle_11.2/mnt/fiddle.sh

<<'EOP' cat > /mnt/fire/oracle_11.2/mnt/probe.c
...
EOP
<<'EOC' cat > /mnt/fire/oracle_11.2/mnt/fiddle.c
...
EOC
OH=/u01/app/oracle/product/11.2.0/xe
chroot /mnt/fire/oracle_11.2/mnt /usr/bin/gcc -O2 -o /probe /probe.c -I$OH/rdbms/public -L$OH/lib -lclntsh
CSID=$(chroot /mnt/fire/oracle_11.2/mnt /usr/bin/env ORACLE_HOME=$OH /probe)
chroot /mnt/fire/oracle_11.2/mnt /usr/bin/gcc -O2 -Wall -DAL32UTF8_ID=$CSID -o /fiddle /fiddle.c \
  -I$OH/rdbms/public -L$OH/lib -lclntsh
rm -f /mnt/fire/oracle_11.2/mnt/fiddle.c /mnt/fire/oracle_11.2/mnt/probe.c /mnt/fire/oracle_11.2/mnt/probe
chroot /mnt/fire/oracle_11.2/mnt /usr/bin/ldd /fiddle

install -m 755 /mnt/fire/oracle_11.2/vsock /mnt/fire/oracle_11.2/mnt/vsock

umount /mnt/fire/oracle_11.2/mnt
umount /mnt/oracle11xe

# read the headroom after the umount: before it the superblock still shows the empty image
free_mb=$(dumpe2fs -h /mnt/fire/oracle_11.2/rootfs.ext4 2>/dev/null \
...
zfs set recordsize=16K tank/fire/oracle_11.2

( cd /mnt/fire/oracle_11.2
  rm -f mem vmstate v.sock* /tmp/fc-snap-oracle_11.2.sock /tmp/fc-snap-oracle_11.2.log
  firecracker-1.13 --api-sock /tmp/fc-snap-oracle_11.2.sock --config-file config.json > /tmp/fc-snap-oracle_11.2.log 2>&1 &
  until grep -q FIDDLE-READY /tmp/fc-snap-oracle_11.2.log ; do sleep 0.1 ; done
  curl -sf --unix-socket /tmp/fc-snap-oracle_11.2.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
  curl -sf --unix-socket /tmp/fc-snap-oracle_11.2.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
  kill $! || true )
# a cold snapshot just serves slowly: check the warm-up ran
grep -a 'warm-up:' /tmp/fc-snap-oracle_11.2.log

# never -R: on a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/oracle_11.2@base 2>/dev/null || true
zfs snapshot tank/fire/oracle_11.2@base

cd /
out=$(echo '["select 1 from dual"]' | /mnt/fire/oracle_11.2/run.sh) || true
printf '%s\n' "$out"
