echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/firebird_4.0/run.sh' >> /etc/sudoers
zfs create tank/fire/firebird_4.0
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/firebird_4.0/vmlinux.bin

<<'EOF' cat > /mnt/fire/firebird_4.0/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/firebird_4.0/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/firebird_4.0/vsock /mnt/fire/firebird_4.0/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/firebird_4.0/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/firebird_4.0/run.sh

mkdir /mnt/fire/firebird_4.0/mnt
...
<<'EODOCKER' cat > /mnt/fire/firebird_4.0/DOCKERFILE
FROM debian:trixie-slim
RUN apt-get update && apt-get install --no-install-recommends -y \
      systemd-sysv udev haveged ca-certificates curl libicu76 libtommath1 libncurses6 libatomic1
RUN curl -sfLo /tmp/fb.tar.gz https://github.com/FirebirdSQL/firebird/releases/download/v4.0.7/Firebird-4.0.7.3271-0.amd64.tar.gz \
 && echo "d2964dfb190d7b0ec57271b53e6539f715ecc119f1cb8cd10339662eed9a7dea  /tmp/fb.tar.gz" | sha256sum -c - \
 && tar xzf /tmp/fb.tar.gz -C /tmp \
 && tar xzf /tmp/Firebird-4.0.7.3271-0.amd64/buildroot.tar.gz -C / \
 && rm -rf /tmp/fb.tar.gz /tmp/Firebird-4.0.7.3271-0.amd64 \
...
# ---- the fiddle runner ----
<<'EOC' cat > /mnt/fire/firebird_4.0/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_firebird_4.0 - < /mnt/fire/firebird_4.0/DOCKERFILE

# 320M: a 229M tree plus fiddle.fdb and ~8M of ext4 overhead leaves 81M free
dd if=/dev/zero bs=1M count=320 > /mnt/fire/firebird_4.0/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/firebird_4.0/rootfs.ext4
mount -o loop /mnt/fire/firebird_4.0/rootfs.ext4 /mnt/fire/firebird_4.0/mnt
docker run --rm -ti -v /mnt/fire/firebird_4.0/mnt:/my-rootfs dummy_firebird_4.0
...
cp /mnt/fire/firebird_4.0/fiddle.c /mnt/fire/firebird_4.0/mnt/fiddle.c
docker run --rm -v /mnt/fire/firebird_4.0/mnt:/my-rootfs debian:trixie-slim bash -c '
  set -e
...
chroot /mnt/fire/firebird_4.0/mnt /usr/bin/ldd /fiddle

docker rmi dummy_firebird_4.0

cp /mnt/fire/firebird_4.0/vsock /mnt/fire/firebird_4.0/mnt/vsock
chmod 755 /mnt/fire/firebird_4.0/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/firebird_4.0/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/firebird_4.0/mnt/fiddle.sh

umount /mnt/fire/firebird_4.0/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/firebird_4.0/rootfs.ext4 2>/dev/null | grep -E 'Block (count|size)|Free blocks'
blk=$(dumpe2fs -h /mnt/fire/firebird_4.0/rootfs.ext4 2>/dev/null | awk '/^Block size:/{print $3}')
free_mb=$(( $(dumpe2fs -h /mnt/fire/firebird_4.0/rootfs.ext4 2>/dev/null | awk '/^Free blocks:/{print $3}') * blk / 1048576 ))
...
zfs set recordsize=16K tank/fire/firebird_4.0

cd /mnt/fire/firebird_4.0
rm -f mem vmstate v.sock* /tmp/fc-snap-firebird_4.0.sock /tmp/fc-snap-firebird_4.0.log
firecracker-1.13 --api-sock /tmp/fc-snap-firebird_4.0.sock --config-file config.json > /tmp/fc-snap-firebird_4.0.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-firebird_4.0.log ; do sleep 0.1 ; done
# proves the warm-up ran: a runner that failed to compile shows up nowhere else
grep -a 'warm-up:' /tmp/fc-snap-firebird_4.0.log
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-firebird_4.0.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-firebird_4.0.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/firebird_4.0@base 2>/dev/null || true
zfs snapshot tank/fire/firebird_4.0@base

out=$(echo '["select rdb$get_context('"'"'SYSTEM'"'"','"'"'ENGINE_VERSION'"'"') as v from rdb$database"]' | /mnt/fire/firebird_4.0/run.sh) || true
printf '%s\n' "$out"
