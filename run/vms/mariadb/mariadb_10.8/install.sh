echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/mariadb_10.8/run.sh' >> /etc/sudoers
zfs create tank/fire/mariadb_10.8
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/mariadb_10.8/vmlinux.bin

<<'EOF' cat > /mnt/fire/mariadb_10.8/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/mariadb_10.8/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/mariadb_10.8/vsock /mnt/fire/mariadb_10.8/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/mariadb_10.8/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/mariadb_10.8/run.sh

mkdir /mnt/fire/mariadb_10.8/mnt

# ---- which MariaDB, and from where ----
<<'EODOCKER' cat > /mnt/fire/mariadb_10.8/DOCKERFILE
...
RUN install -d /etc/apt/keyrings \
 ...
 && echo 'deb [signed-by=/etc/apt/keyrings/mariadb.asc] https://archive.mariadb.org/mariadb-10.8.8/repo/debian bullseye main' \
      > /etc/apt/sources.list.d/mariadb.list \
 && apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y \
      mariadb-server=1:10.8.8+maria~deb11 \
      mariadb-client=1:10.8.8+maria~deb11
# the frozen repo carries other patch releases too: fail on any package that is not 10.8.8
RUN dpkg-query -W -f='${Package} ${Version}\n' | grep -E '^(mariadb|libmariadb)' ; \
    bad=$(dpkg-query -W -f='${Package} ${Version}\n' | grep -E '^(mariadb|libmariadb)' \
          | grep -v '1:10\.8\.8+maria~deb11' || true) ; \
    [ -z "$bad" ] || { echo "FAIL: non-10.8.8 mariadb package installed: $bad" ; exit 1 ; }
...
# to every copy
<<'EOC' cat > /mnt/fire/mariadb_10.8/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <mysql.h>
...
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_mariadb_10.8 - < /mnt/fire/mariadb_10.8/DOCKERFILE

# re-derive this count per engine; never copy a sibling's
dd if=/dev/zero bs=1M count=567 > /mnt/fire/mariadb_10.8/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/mariadb_10.8/rootfs.ext4
mount -o loop /mnt/fire/mariadb_10.8/rootfs.ext4 /mnt/fire/mariadb_10.8/mnt
docker run --rm -ti -v /mnt/fire/mariadb_10.8/mnt:/my-rootfs dummy_mariadb_10.8
...
cp /mnt/fire/mariadb_10.8/fiddle.c /mnt/fire/mariadb_10.8/mnt/fiddle.c
docker run --rm -v /mnt/fire/mariadb_10.8/mnt:/my-rootfs debian:bullseye-slim bash -c '
  set -e
  apt-get update -qq && apt-get install -y -qq --no-install-recommends ca-certificates curl
  install -d /etc/apt/keyrings
  curl -sfLo /etc/apt/keyrings/mariadb.asc https://archive.mariadb.org/PublicKey
  echo "00adb387f4c12e8982cfc28165d225695621494354cd58108c6845063bfb74b5  /etc/apt/keyrings/mariadb.asc" | sha256sum -c -
  echo "deb [signed-by=/etc/apt/keyrings/mariadb.asc] https://archive.mariadb.org/mariadb-10.8.8/repo/debian bullseye main" > /etc/apt/sources.list.d/mariadb.list
  apt-get update -qq
  apt-get install -y -qq --no-install-recommends gcc libc6-dev libmariadb-dev=1:10.8.8+maria~deb11
  gcc -O2 -Wall -o /my-rootfs/fiddle /my-rootfs/fiddle.c $(mariadb_config --cflags --libs)
  rm /my-rootfs/fiddle.c'
chroot /mnt/fire/mariadb_10.8/mnt /usr/bin/ldd /fiddle

docker rmi dummy_mariadb_10.8

cp /mnt/fire/mariadb_10.8/vsock /mnt/fire/mariadb_10.8/mnt/vsock
chmod 755 /mnt/fire/mariadb_10.8/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/mariadb_10.8/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/mariadb_10.8/mnt/fiddle.sh

umount /mnt/fire/mariadb_10.8/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/mariadb_10.8/rootfs.ext4 2>/dev/null | grep -E 'Block (count|size)|Free blocks'
blk=$(dumpe2fs -h /mnt/fire/mariadb_10.8/rootfs.ext4 2>/dev/null | awk '/^Block size:/{print $3}')
free_mb=$(( $(dumpe2fs -h /mnt/fire/mariadb_10.8/rootfs.ext4 2>/dev/null | awk '/^Free blocks:/{print $3}') * blk / 1048576 ))
...
zfs set recordsize=16K tank/fire/mariadb_10.8

cd /mnt/fire/mariadb_10.8
rm -f mem vmstate v.sock* /tmp/fc-snap-mariadb_10.8.sock /tmp/fc-snap-mariadb_10.8.log
firecracker-1.13 --api-sock /tmp/fc-snap-mariadb_10.8.sock --config-file config.json > /tmp/fc-snap-mariadb_10.8.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-mariadb_10.8.log ; do sleep 0.1 ; done
# must show a result, or the guest snapshotted with no reachable server
grep -a 'warm-up:' /tmp/fc-snap-mariadb_10.8.log
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-mariadb_10.8.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-mariadb_10.8.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/mariadb_10.8@base 2>/dev/null || true
zfs snapshot tank/fire/mariadb_10.8@base

out=$(echo '["select version()"]' | /mnt/fire/mariadb_10.8/run.sh) || true
printf '%s\n' "$out"
