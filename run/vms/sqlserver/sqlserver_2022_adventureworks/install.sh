# mssql-server comes from the mssql-server-2022 channel, which cannot cross into a later
# release. Never take it from prod.list's unversioned package.
echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/sqlserver_2022_adventureworks/run.sh' >> /etc/sudoers
zfs create tank/fire/sqlserver_2022_adventureworks
cp /mnt/fire/vmlinux-6.1.141 /mnt/fire/sqlserver_2022_adventureworks/vmlinux.bin
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/cpu_template.json
{
  "cpuid_modifiers": [
    {
      "leaf": "0x80000007",
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2022_adventureworks/vsock /mnt/fire/sqlserver_2022_adventureworks/vsock.c
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/reseedrng.c
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define TABLE_RVA 0x7c860          /* bcryptPrimitives.dll .data: per-processor DRBG table */
...
EOF
gcc -O2 -static -o /mnt/fire/sqlserver_2022_adventureworks/reseedrng /mnt/fire/sqlserver_2022_adventureworks/reseedrng.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/sqlserver_2022_adventureworks/run.sh

mkdir /mnt/fire/sqlserver_2022_adventureworks/mnt
mkdir /mnt/fire/sqlserver_2022_adventureworks/build
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/build/fiddle.csproj
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/build/fiddle.cs
using System;
...
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/build/Dockerfile
...
docker build -t dummy_sqlserver_2022_adventureworks /mnt/fire/sqlserver_2022_adventureworks/build
dd if=/dev/zero bs=1M count=8192 > /mnt/fire/sqlserver_2022_adventureworks/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/sqlserver_2022_adventureworks/rootfs.ext4
mount -o loop /mnt/fire/sqlserver_2022_adventureworks/rootfs.ext4 /mnt/fire/sqlserver_2022_adventureworks/mnt
docker run --rm -i -v /mnt/fire/sqlserver_2022_adventureworks/mnt:/my-rootfs dummy_sqlserver_2022_adventureworks -s <<"SETUP"
...
docker rmi dummy_sqlserver_2022_adventureworks

cp /mnt/fire/sqlserver_2022_adventureworks/vsock /mnt/fire/sqlserver_2022_adventureworks/mnt/vsock
chmod 755 /mnt/fire/sqlserver_2022_adventureworks/mnt/vsock
cp /mnt/fire/sqlserver_2022_adventureworks/reseedrng /mnt/fire/sqlserver_2022_adventureworks/mnt/reseedrng
chmod 755 /mnt/fire/sqlserver_2022_adventureworks/mnt/reseedrng

# replace docker's bind-mounted network identity with the guest's offline one
printf '127.0.0.1 localhost fiddle\n::1 localhost\n' > /mnt/fire/sqlserver_2022_adventureworks/mnt/etc/hosts
echo fiddle > /mnt/fire/sqlserver_2022_adventureworks/mnt/etc/hostname
: > /mnt/fire/sqlserver_2022_adventureworks/mnt/etc/resolv.conf
rm -f /mnt/fire/sqlserver_2022_adventureworks/mnt/etc/systemd/system/multi-user.target.wants/mssql-server.service
...
mkdir -p /mnt/fire/sqlserver_2022_adventureworks/mnt/awdata
rm -rf /tmp/awcsv-sqlserver_2022_adventureworks
unzip -q -o -j /tmp/AdventureWorks-oltp-install-script.zip '*.csv' -d /tmp/awcsv-sqlserver_2022_adventureworks
ls /tmp/awcsv-sqlserver_2022_adventureworks/*.csv | wc -l | grep -qx 69 || { echo 'expected 69 csv data files'; exit 1; }
...
for f in /tmp/awcsv-sqlserver_2022_adventureworks/*.csv; do
  o=/mnt/fire/sqlserver_2022_adventureworks/mnt/awdata/$(basename "$f")
  printf '\377\376' > "$o"
  iconv -f UTF-8 -t UTF-16LE < "$f" >> "$o" || { echo "iconv failed on $f"; exit 1; }
done
rm -rf /tmp/awcsv-sqlserver_2022_adventureworks
ls /mnt/fire/sqlserver_2022_adventureworks/mnt/awdata/*.csv | wc -l | grep -qx 69 || { echo 'expected 69 csv data files'; exit 1; }
for f in /mnt/fire/sqlserver_2022_adventureworks/mnt/awdata/*.csv; do
  [ "$(head -c2 "$f" | od -An -tx1 | tr -d ' ')" = fffe ] || { echo "missing BOM: $f"; exit 1; }
done
# BULK INSERT needs the trailing separator
sed 's|__DATAPATH__|/awdata/|' adventureworks/instawdb.linux.sql > /mnt/fire/sqlserver_2022_adventureworks/mnt/instawdb.sql
grep -q '__DATAPATH__' /mnt/fire/sqlserver_2022_adventureworks/mnt/instawdb.sql && { echo 'datapath substitution failed'; exit 1; }

# ---- init boot ----
<<'EOF' cat > /mnt/fire/sqlserver_2022_adventureworks/mnt/fiddle.sh
#!/bin/sh
...
reboot -ff
EOF
chmod 700 /mnt/fire/sqlserver_2022_adventureworks/mnt/fiddle.sh
umount /mnt/fire/sqlserver_2022_adventureworks/mnt

cd /mnt/fire/sqlserver_2022_adventureworks
rm -f /tmp/fc-init-sqlserver_2022_adventureworks.log
firecracker-1.13 --no-api --config-file config.json > /tmp/fc-init-sqlserver_2022_adventureworks.log 2>&1
grep FIDDLE-INIT-DONE /tmp/fc-init-sqlserver_2022_adventureworks.log   # must appear; if not, read the log
# hard gates: a partial load can leave the database looking entirely normal
L=/tmp/fc-init-sqlserver_2022_adventureworks.log
...
grep -a 'FIDDLE-DB-STATE-RC=0' /tmp/fc-init-sqlserver_2022_adventureworks.log  # ...with snapshot isolation and a master key
...
dumpe2fs -h /mnt/fire/sqlserver_2022_adventureworks/rootfs.ext4 2>/dev/null | grep -E 'Block count|Free blocks|Block size|Reserved block count'
...
rm -f mem vmstate v.sock* /tmp/fc-snap-sqlserver_2022_adventureworks.sock /tmp/fc-snap-sqlserver_2022_adventureworks.log
firecracker-1.13 --api-sock /tmp/fc-snap-sqlserver_2022_adventureworks.sock --config-file config.json > /tmp/fc-snap-sqlserver_2022_adventureworks.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-sqlserver_2022_adventureworks.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2022_adventureworks.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-sqlserver_2022_adventureworks.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true
...
grep -a SQLSERVR-WARM /tmp/fc-snap-sqlserver_2022_adventureworks.log       # must appear, BEFORE FIDDLE-READY
grep -a 'Kernel command line' /tmp/fc-snap-sqlserver_2022_adventureworks.log | grep -a no-kvmapf   # must match
grep -a 'Unknown kernel command line parameters' /tmp/fc-snap-sqlserver_2022_adventureworks.log    # must NOT match
grep -a CLOCK-AGED /tmp/fc-snap-sqlserver_2022_adventureworks.log          # randomness lever 1 ran
grep -a 'RESEEDRNG: .* states re-keyed' /tmp/fc-snap-sqlserver_2022_adventureworks.log   # lever 2's layout still matches
grep -a 'RESEEDRNG: .* 0 states re-keyed' /tmp/fc-snap-sqlserver_2022_adventureworks.log && echo 'CEREMONY FAILED: CNG layout no longer matches'

# leaked clones block the destroy, leaving a stale @base the snapshot line then trips on
zfs destroy tank/fire/sqlserver_2022_adventureworks@base 2>/dev/null || true
zfs snapshot tank/fire/sqlserver_2022_adventureworks@base || echo 'CEREMONY FAILED: stale @base still in place (leaked clones?)'

screen_bad=0
for i in $(seq 1 $SCREEN_N); do
  [ -n "$(echo '["select 1"]' | /mnt/fire/sqlserver_2022_adventureworks/run.sh 2>/dev/null)" ] || screen_bad=$((screen_bad + 1))
done
...
out=$(echo '["select @@version"]' | /mnt/fire/sqlserver_2022_adventureworks/run.sh) || true
printf '%s\n' "$out"
