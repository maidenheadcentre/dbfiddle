echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/sqlite_3.39/run.sh' >> /etc/sudoers
zfs create tank/fire/sqlite_3.39
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/sqlite_3.39/vmlinux.bin

<<'EOF' cat > /mnt/fire/sqlite_3.39/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/sqlite_3.39/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/sqlite_3.39/vsock /mnt/fire/sqlite_3.39/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/sqlite_3.39/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/sqlite_3.39/run.sh

mkdir /mnt/fire/sqlite_3.39/mnt

<<'EODOCKER' cat > /mnt/fire/sqlite_3.39/DOCKERFILE
...
<<'EOC' cat > /mnt/fire/sqlite_3.39/fiddle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "sqlite3.h"
...
mkdir -p /mnt/fire/sqlite_3.39/src
curl -sfLo /mnt/fire/sqlite_3.39/src/sqlite-amalgamation-3390400.zip \
     https://www.sqlite.org/2022/sqlite-amalgamation-3390400.zip
<<'EOF' sha256sum -c -
9c99955b21d2374f3a385d67a1f64cbacb1d4130947473d25c77ad609c03b4cd  /mnt/fire/sqlite_3.39/src/sqlite-amalgamation-3390400.zip
EOF
unzip -o -j -d /mnt/fire/sqlite_3.39/src \
      /mnt/fire/sqlite_3.39/src/sqlite-amalgamation-3390400.zip
grep -m1 'define SQLITE_VERSION ' /mnt/fire/sqlite_3.39/src/sqlite3.h

# per-engine tag: concurrent builds on cumbria2 must never share one
DOCKER_BUILDKIT=1 docker build --no-cache -t dummy_sqlite_3.39 - < /mnt/fire/sqlite_3.39/DOCKERFILE
# 240M: a 143M tree plus /fiddle and /vsock leaves 89M free
dd if=/dev/zero bs=1M count=240 > /mnt/fire/sqlite_3.39/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/sqlite_3.39/rootfs.ext4
mount -o loop /mnt/fire/sqlite_3.39/rootfs.ext4 /mnt/fire/sqlite_3.39/mnt
docker run --rm -ti -v /mnt/fire/sqlite_3.39/mnt:/my-rootfs dummy_sqlite_3.39
...
cp /mnt/fire/sqlite_3.39/fiddle.c /mnt/fire/sqlite_3.39/mnt/fiddle.c
cp /mnt/fire/sqlite_3.39/src/sqlite3.c /mnt/fire/sqlite_3.39/src/sqlite3.h /mnt/fire/sqlite_3.39/mnt/
docker run --rm -v /mnt/fire/sqlite_3.39/mnt:/my-rootfs debian:trixie-slim bash -c '
...
chroot /mnt/fire/sqlite_3.39/mnt /usr/bin/ldd /fiddle

docker rmi dummy_sqlite_3.39

cp /mnt/fire/sqlite_3.39/vsock /mnt/fire/sqlite_3.39/mnt/vsock
chmod 755 /mnt/fire/sqlite_3.39/mnt/vsock

# any edit to fiddle.sh or /fiddle invalidates the snapshot: re-run the ceremony
cat <<"EOF" > /mnt/fire/sqlite_3.39/mnt/fiddle.sh
#!/bin/sh
...
chmod 700 /mnt/fire/sqlite_3.39/mnt/fiddle.sh

umount /mnt/fire/sqlite_3.39/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/sqlite_3.39/rootfs.ext4 2>/dev/null | grep -E 'Block (count|size)|Free blocks'
blk=$(dumpe2fs -h /mnt/fire/sqlite_3.39/rootfs.ext4 2>/dev/null | awk '/^Block size:/{print $3}')
free_mb=$(( $(dumpe2fs -h /mnt/fire/sqlite_3.39/rootfs.ext4 2>/dev/null | awk '/^Free blocks:/{print $3}') * blk / 1048576 ))
...
zfs set recordsize=16K tank/fire/sqlite_3.39

cd /mnt/fire/sqlite_3.39
rm -f mem vmstate v.sock* /tmp/fc-snap-sqlite_3.39.sock /tmp/fc-snap-sqlite_3.39.log
firecracker-1.13 --api-sock /tmp/fc-snap-sqlite_3.39.sock --config-file config.json > /tmp/fc-snap-sqlite_3.39.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-sqlite_3.39.log ; do sleep 0.1 ; done
# proves the warm-up ran: a runner that failed to compile shows up nowhere else
grep -a 'warm-up:' /tmp/fc-snap-sqlite_3.39.log
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-sqlite_3.39.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-sqlite_3.39.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/sqlite_3.39@base 2>/dev/null || true
zfs snapshot tank/fire/sqlite_3.39@base

out=$(echo '["select sqlite_version()"]' | /mnt/fire/sqlite_3.39/run.sh) || true
printf '%s\n' "$out"
