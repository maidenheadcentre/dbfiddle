echo 'www-data        ALL=(ALL) NOPASSWD: /mnt/fire/postgres_9.6/run.sh' >> /etc/sudoers
zfs create tank/fire/postgres_9.6
cp /mnt/fire/vmlinux-5.10.223 /mnt/fire/postgres_9.6/vmlinux.bin

<<'EOF' cat > /mnt/fire/postgres_9.6/config.json
{
  "boot-source": {
...
<<'EOF' cat > /mnt/fire/postgres_9.6/vsock.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
EOF
gcc -O2 -static -o /mnt/fire/postgres_9.6/vsock /mnt/fire/postgres_9.6/vsock.c

# never write listener.pl here: /mnt/fire/listener.pl is shared by every engine

<<'EOF' cat > /mnt/fire/postgres_9.6/run.sh
#!/bin/bash
...
chmod 700 /mnt/fire/postgres_9.6/run.sh

mkdir /mnt/fire/postgres_9.6/mnt

<<'EODOCKER' cat > /mnt/fire/postgres_9.6/DOCKERFILE
FROM alpine:3.16
...
RUN set -eux; \
	...
	cp /usr/share/automake-*/config.guess /usr/share/automake-*/config.sub config/; \
	config/config.sub "$gnuArch"; \
	./configure \
		--build="$gnuArch" \
...
ENV PGDATA /usr/postgres

STOPSIGNAL SIGINT

# loopback: without lo the postmaster cannot bind its stats socket and runs with autovacuum
# silently off. datestyle and timezone override initdb's 'ISO, MDY' and 'UCT', which would
# change every date a fiddle prints.
RUN echo "root:Docker!" | chpasswd \
  && apk add --no-cache openrc util-linux haveged \
  ...
  && printf '%s\n' \
       ...
       "map           root              postgres" > $PGDATA/pg_ident.conf \
  && printf '%s\n' \
       "#TYPE   DATABASE    USER        CIDR-ADDRESS          METHOD" \
       "local   all         postgres                          ident map=map" > $PGDATA/pg_hba.conf \
  && chown postgres:postgres $PGDATA/postgresql.conf $PGDATA/pg_ident.conf $PGDATA/pg_hba.conf \
  && echo '#!/sbin/openrc-run' > /etc/init.d/pg \
  && echo 'command="pg_ctl"' >> /etc/init.d/pg \
  && echo 'command_args="start -D /usr/postgres"' >> /etc/init.d/pg \
  && echo 'pidfile="/usr/postgres/postmaster.pid"' >> /etc/init.d/pg \
  && echo 'command_user="postgres:postgres"' >> /etc/init.d/pg \
  && chmod 700 /etc/init.d/pg \
  && rc-update add pg default \
  && echo '#!/sbin/openrc-run' > /etc/init.d/fiddle \
  && echo 'command=/fiddle.sh' >> /etc/init.d/fiddle \
  && echo 'rc_use="pg"' >> /etc/init.d/fiddle \
  && chmod 700 /etc/init.d/fiddle \
  && rc-update add fiddle default
ENTRYPOINT ["sh"]
EODOCKER

# per-engine tag: concurrent builds on cumbria2 must never share one
docker build --no-cache -t dummy_postgres_9.6 - < /mnt/fire/postgres_9.6/DOCKERFILE
#docker run --rm -ti dummy_postgres_9.6
# 243 = 168M occupied by the populated image + ~75M free. Measure the occupancy from the
# superblock after a throwaway populate, never with du in the builder container.
dd if=/dev/zero bs=1M count=243 > /mnt/fire/postgres_9.6/rootfs.ext4
mkfs.ext4 -m 0 /mnt/fire/postgres_9.6/rootfs.ext4
mount -o loop /mnt/fire/postgres_9.6/rootfs.ext4 /mnt/fire/postgres_9.6/mnt
docker run --rm -ti -v /mnt/fire/postgres_9.6/mnt:/my-rootfs dummy_postgres_9.6
for d in bin etc lib root sbin usr; do tar c "/$d" | tar x -C /my-rootfs; done
for dir in dev var tmp; do mkdir /my-rootfs/${dir}; done
chmod 1777 /my-rootfs/tmp
mkdir -p /my-rootfs/var/run/postgresql
chown postgres:postgres /my-rootfs/var/run/postgresql
...
cat <<"EOF" > /mnt/fire/postgres_9.6/mnt/fiddle.sh
#!/bin/sh
dd if=/dev/random count=1 bs=1
until [ -S /var/run/postgresql/.s.PGSQL.5432 ] ; do sleep 0.1 ; done
until /usr/local/bin/pg_isready -h localhost -p 5432 >/dev/null 2>&1 ; do sleep 0.05 ; done
...
until /usr/local/bin/pg_isready -h localhost -p 5432 ; do sleep 0.05 ; done
/fiddle
/vsock reply < /data/output.json
reboot -ff
EOF

chmod 700 /mnt/fire/postgres_9.6/mnt/fiddle.sh

#mount -o loop /mnt/fire/postgres_9.6/rootfs.ext4 /mnt/fire/postgres_9.6/mnt
<<'EOC' cat > /mnt/fire/postgres_9.6/fiddle.c
...
cp /mnt/fire/postgres_9.6/fiddle.c /mnt/fire/postgres_9.6/mnt/fiddle.c
docker run --rm -v /mnt/fire/postgres_9.6/mnt:/my-rootfs dummy_postgres_9.6 -c 'apk add --no-cache gcc musl-dev >/dev/null && gcc -O2 -Wall -I$(pg_config --includedir) -o /my-rootfs/fiddle /my-rootfs/fiddle.c -L$(pg_config --libdir) -lpq && rm /my-rootfs/fiddle.c'
chroot /mnt/fire/postgres_9.6/mnt /usr/bin/ldd /fiddle
chroot /mnt/fire/postgres_9.6/mnt /usr/bin/ldd /fiddle | grep -q 'libpq.so.5 => /usr/local/lib/libpq.so.5' \
  || { echo "ABORT: /fiddle did not resolve the guest's libpq"; exit 1; }
# drop the image: cumbria2's root filesystem fills otherwise
docker rmi dummy_postgres_9.6

cp /mnt/fire/postgres_9.6/vsock /mnt/fire/postgres_9.6/mnt/vsock
chmod 755 /mnt/fire/postgres_9.6/mnt/vsock

umount /mnt/fire/postgres_9.6/mnt

# read the headroom after the umount: before it the superblock still shows the empty image
dumpe2fs -h /mnt/fire/postgres_9.6/rootfs.ext4 2>/dev/null | awk '
...
# ^ without `|| exit 1` a failed assert carries on into the snapshot

zfs set recordsize=16K tank/fire/postgres_9.6

cd /mnt/fire/postgres_9.6
rm -f mem vmstate v.sock* /tmp/fc-snap-postgres_9.6.sock /tmp/fc-snap-postgres_9.6.log
firecracker-1.13 --api-sock /tmp/fc-snap-postgres_9.6.sock --config-file config.json > /tmp/fc-snap-postgres_9.6.log 2>&1 &
until grep -q FIDDLE-READY /tmp/fc-snap-postgres_9.6.log ; do sleep 0.1 ; done
sleep 0.3
curl -sf --unix-socket /tmp/fc-snap-postgres_9.6.sock -X PATCH http://localhost/vm -H 'Content-Type: application/json' -d '{"state":"Paused"}'
curl -sf --unix-socket /tmp/fc-snap-postgres_9.6.sock -X PUT http://localhost/snapshot/create -H 'Content-Type: application/json' -d '{"snapshot_type":"Full","snapshot_path":"vmstate","mem_file_path":"mem"}'
kill $! || true

# never -R: against a live engine it destroys the clones of in-flight fiddles
zfs destroy tank/fire/postgres_9.6@base 2>/dev/null || true
zfs snapshot tank/fire/postgres_9.6@base

out=$(echo '["select 1"]' | /mnt/fire/postgres_9.6/run.sh) || true
printf '%s\n' "$out"
